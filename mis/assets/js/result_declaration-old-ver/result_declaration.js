 ...
 function  save_resut_declaration(){  
    if (confirm("Are you Sure?")){             
       ...
       var postdata = { 
              ...
              $.ajax({
                        url: site_url('result_declaration-old-ver/result_declaration_drside/save_data'),
                        type: "POST",
                        dataType: "json",
                        data: postdata,                       
                            ...
                            if (jsonObj.result === "Successfully") {                                                                                                                                                                                     
                               $("#msg").removeClass().addClass("alert alert-success");
                                ...
                                $("#msg").append(" By DR-Exam ");                                
                                 setTimeout(function () {
                                if (jsonObj.result === "Successfully") {  
                                window.location.href = site_url('result_declaration-old-ver/result_declaration_drside/show_details/'+$('#hsyear').val()+'/'+$('#hsess').val()+'/'+$('#hetype').val()+'/'+$('#hdeptid').val()+'/F/'+$('#hsec_name').val());                            
                                /*$("#session_year_attendance").val($('#hsyear').val());                                 
                                $("#session_attendance").val($('#hsess').val());                                 
                                $("#exm_type").val($('#hetype').val());                                 
                                $("#dept").val($('#hdeptid').val());                                 
                                $("#rd_type").val('F');                                 */
                                 }                             
                                $("#msg").hide();                                  
                            }, 5000);
