 ...
 */
    ...
    function view(rd,admn_no,sess, syear,dept, course_id,branch_id, sem,section, name,exam_type){
        $.ajax({            
            url: site_url('result_declaration/result_declaration_drside/view_result'),
            type: "POST",
            async: false,
            data: {rd:rd,"admn_no": admn_no, "session": sess, "session_year": syear, "course_id": course_id, "branch_id": branch_id, "semester": sem, 'name': name,'dept':dept,'exam_type':exam_type,'section':section, published: $("#published").val(),actual_published: $("#actual_published").val(),status:$("#status").val()},
    ...
    function saveRe(rdec_type) {
        ...
        if ((rdec_type == 'prerd' ? $('#datere_pre').val() : $('#datere').val()) == "") {
            (rdec_type == 'prerd' ? $("#rerror_pre") : $("#rerror")).html("Please Enter Re-Declartion Date First !").show();
            setTimeout(function () {
                (rdec_type == 'prerd' ? $("#rerror_pre") : $("#rerror")).fadeOut();
            }, 1000);
            return false;
        } else if ((rdec_type == 'prerd' ? $("#rreason_pre").val() : $("#rreason").val()) == "") {
            (rdec_type == 'prerd' ? $("#rerror_pre") : $("#rerror")).html("Please Enter Re-Declartion Reason !").show();
            setTimeout(function () {
                (rdec_type == 'prerd' ? $("#rerror_pre") : $("#rerror")).fadeOut();
            }, 1000);
            return false;
        }
          else if(!$('.class_withhold input[type="checkbox"]').is(':checked')){
              $("#rerror").html("Please check at least one checkbox to Re-Declare!").show();
           return false;
       } 
                ...
                success: function (r) {
                    ...
                    if (Boolean(r.status) == true) {
                      ...
                      //  if (rdec_type == 'postrd') {
                            ...
                            $('#msg').html(" <a href='#' class='close' data-dismiss='alert'>&times;</a><i class='fa fa-danger'></i><strong>" + (rdec_type == 'prerd' ? 'Pre-Withhold Redeclaration' : 'Post-Withhold Redeclaration ') + " failed for selected candidates"); 
                            setTimeout(function () {
                             $("#msg").hide();                                
                             }, 5000);
                             return false;
    ...
    $(document).ready(function () {
        ...
        $('#save_all').on('click', function () {
            ...
            if (Boolean(bool)) {
                $("#msg").removeClass().addClass("alert alert-success");
                $("#msg").html(" <a href='#' class='close' data-dismiss='alert'>&times;</a><i class='fa fa-check'></i><strong>Process of " + ($("#rdec_type").val() == 'prerd' ? 'Pre-Withhold' : 'Post-Withhold') + " done successfully for selected candidates [" + chk1 + "]  </strong>for <b>Session_yr:</b>" + sy + ",<b>Session:</b>" + sess + ",<b>Exam type:</b>" + et + ",<b>Department:</b>" + $("#dptname").val() + " " + ((sec) ? ",<b>Section:</b>" + sec + "" : "") + ",<b>Course:</b>" + $("#crs_name").val() + ",<b>Branch:</b>" + $("#brn_name").val() + ",<b>Semseter:</b>" + sem + "");
            }
            else {
                $("#msg").removeClass().addClass("alert alert-danger");
                $("#msg").html(" <a href='#' class='close' data-dismiss='alert'>&times;</a><i class='fa fa-exclamation'></i><strong>Process of " + ($("#rdec_type").val() == 'prerd' ? 'Pre-Withhold' : 'Post-Withhold') + " Failed either due candidates were  already " + ($("#rdec_type").val() == 'prerd' ? 'Pre-Withhold' : 'Post-Withhold') + " from before or Internal error occured, for selected candidates</strong> for  <b>Session_yr:</b>" + sy + ",<b>Session:</b>" + sess + ",<b>Exam type:</b>" + et + ",<b>Department:</b>" + $("#dptname").val() + " " + ((sec) ? ",<b>Section:</b>" + sec + "" : "") + ",<b>Course:</b>" + $("#crs_name").val() + ",<b>Branch:</b>" + $("#brn_name").val() + ",<b>Semseter:</b>" + sem + "");
            }
        });
    ...
    // check all chekbox in single checkbox click in first page only
  ...
  function checkAll(ele) {    
    ...
    // check all chekbox in single checkbox click in all paginated pages. checkAll function and this one combindly works great
   $("#ex_mod1_paginate").click(function () {
  if($('#selectAll').is(":checked")){
    $(".wh").prop("checked", "true");
  }
  else {
    $(".wh").removeAttr('checked');
  }
});
