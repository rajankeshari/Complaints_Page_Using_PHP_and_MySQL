$(document).ready(function(){
	...
	$("button#add_sector_button").on("click",function(e){
		$.ajax({
			...
			success : function(data){
				alert("Data added successfully");
				location.reload();
				//$("#add_sector_table").append('<tr><td>'+$("#add_sector_text").val()+'</td><td><button value="Delete" class=" btn btn-danger delete_course" data-course_id="'+$("#course_selection").val()+'" type="button">Delete</button></td></tr>');
			},
			error : function(data){
				alert("Data already exists");
			}
	...
	$("table#add_category_table").on("click","button.delete_category" ,function(e){
		$.ajax({
			url:"./config_jnf/delete_category",
			data: {
