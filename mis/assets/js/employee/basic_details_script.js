	...
	function payband_handler()
	{
		...
		if(r == 'NF_L')
		{
	     var k=pb.substring(9);
		 var y='NF_L'+k;
		}
	    else
	    {
         var k=pb.substring(8);
		 var y='F_L'+k;
		...
		xmlhttp.open("POST",site_url("ajax/grade_pay/"+y),true);
		xmlhttp.send();
...
//AJAX for designation called from teaching_handler
	function designation_dropdown(auth)
	{
		...
		xmlhttp.send();
		document.getElementById("des").innerHTML="<option >-- Please Select--</option>";
	}
	  	...
	  	{ 
	  		if (xmlhttp.readyState==4 && xmlhttp.status==200)
		    {
				...
				else if(xmlhttp.responseText.trim() != '')
				{
					...
					//edit by govind sahu to make nft and nfta selected 
