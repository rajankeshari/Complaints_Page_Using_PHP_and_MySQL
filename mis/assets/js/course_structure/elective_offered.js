...
$(document).ready(function(){
	...
	$aggr_id_selection = $("#aggr_id_selection").hide();
	$aggr_id = $("#aggr_id").hide();
	...
	$branch_selection.on('change',function(){
		...
		$.ajax({url:site_url('course_structure/elective_offered_home/json_get_aggr_id/'),
				method:'POST',
				data:{'branch':$branch_selection.val(),'course':$course.val()},
			success:function(data){
				console.log(data);
				res = jQuery.parseJSON(data);
				
				base_str = '<option value = "0" disabled = "true" selected>Select Effiective From</option>';	
				
				$.each(res,function(key,val){
					vals = val.aggr_id.split('_');
						base_str += '<option value="'+val.aggr_id+'"> '+vals[2]+' - '+vals[3]+'</option>';
					})
				
				$aggr_id_selection.show();
				$aggr_id.html(base_str).show()
				$form_box.hideLoading();
			}
		});
	})
