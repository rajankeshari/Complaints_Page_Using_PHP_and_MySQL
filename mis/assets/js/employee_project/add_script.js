	...
	function onclick_add()
{
	...
	function image_validation()
	{
		...
		{
			alert('The image should be in bmp, png, jpg or jpeg format.');
			return false;
		}
	}
	...
	function idProof_validation()
	{
		...
		{
			alert('The Identity Proof should be in bmp, png, jpg or jpeg format.');
			return false;
		}
	}
	function fetch_details()
	{
		...
		xmlhttp.onreadystatechange=function()
	  	{ 
	  		if (xmlhttp.readyState==4 && xmlhttp.status==200)
		    {
				...
				if(xmlhttp.responseText.trim() == 'NO_EMP')
				{
					//alert("anuj");
					alert('Please select an Employee Id');
					$("#fetch_id_btn").show();
				}
				else if(xmlhttp.responseText.trim() == 'NO_AVAIL')
				{
					alert('Employee Id already in Use. Please select another Employee Id');
					$("#fetch_id_btn").show();
				}
				else if(xmlhttp.responseText.trim() != '')
				{
					//alert('test');
		...
		//xmlhttp.open("POST",site_url("employee/emp_ajax/feedback_emp_detail/"+emp_id),true); // Original
		//xmlhttp.open("POST",site_url("employee/emp_ajax/project_emp_detail/"+emp_id),true);
		xmlhttp.open("POST",site_url("employee_project/add/check_availability_ajax/"+emp_id),true);
		xmlhttp.send();
	}
	...
	/*function fetch_details()
	{
		...
		//alert('i am inside java script fetch_details');
		//alert('i am inside java script fetch_details again ');
		$("#fetch_id_btn").hide();
		//alert('i am inside java script fetch_details again ');
		$("#empIdIcon").show();
		var xmlhttp;
		...
		xmlhttp.onreadystatechange=function()
	  	{ 
	  		if (xmlhttp.readyState==4 && xmlhttp.status==200)
		    {
				...
				{
					alert('Please insert an Employee Id');
					$("#fetch_id_btn").show();
				}
				else if(xmlhttp.responseText.trim() == 'NO_AVAIL')
				{
