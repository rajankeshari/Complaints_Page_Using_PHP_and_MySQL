	//this function select the employee_id on selection of name 
function getEmpId_admin()
	{    
		//alert($('#employee_select').val());
	...
	function onchange_employee_id()
	{
	
		var emp_id=document.getElementById('emp_id').value;
		var view_emp_auth=document.getElementById('view_emp_auth');
        //var view_users=document.getElementById('view_users');
		var xmlhttp;
		if (window.XMLHttpRequest)
		{// code for IE7+, Firefox, Chrome, Opera, Safari
		...
		xmlhttp.onreadystatechange=function()
	  	{
	  		if (xmlhttp.readyState==4 && xmlhttp.status==200)
		    {
			    view_emp_auth.innerHTML = xmlhttp.responseText;
		    }
	  	}
		xmlhttp.open("POST",site_url("super_admin/admin_ajax/getUsersByEmpId/"+emp_id),true);
		xmlhttp.send();
		view_emp_auth.innerHTML = "<center><i class=\"loading\"></i></center>";
	}
	 ...
	 function update_auth(id)
	{
		...
		if(result==true)
		{
			var view_users=document.getElementById('view_user');
			var xmlhttp;
			if (window.XMLHttpRequest)
			{// code for IE7+, Firefox, Chrome, Opera, Safari
			 	xmlhttp=new XMLHttpRequest();
	 ...
	 function update_status(id,auth)
	{   
		var result=confirm("Do you really want to deny the authorization ?");
		if(result==true)
		{
			var view_users=document.getElementById('view_emp_auth');
			var xmlhttp;
			if (window.XMLHttpRequest)
			{// code for IE7+, Firefox, Chrome, Opera, Safari
			 	xmlhttp=new XMLHttpRequest();
			...
			xmlhttp.open("POST",site_url("super_admin/admin_ajax/updateStatus/"+id+"/"+auth),true);
			xmlhttp.send();
			view_users.innerHTML = "<center><i class=\"loading\"></i></center>";
		}
	}
