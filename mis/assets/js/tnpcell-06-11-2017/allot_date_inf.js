...
function checkslot_reschedule($from,$to,$company_id)
{
	...
	$.ajax({
		url:site_url("tnpcell/allot_date_inf/json_get_company_inrange/"+$from+"/"+$to),
...
function reschedule_in_reschedule(company_id,date_from,date_to)
{
	...
	if($("#date_reschedulefrom_"+company_id).val() != "" && $("#date_rescheduleto_"+company_id).val() != "")
		var data_to_send = {'company_id':company_id,'date_from':date_from,'date_to':date_to,'stu_visibility':stu_visibility};		
	else 
		var data_to_send = {'company_id':company_id,'stu_visibility':stu_visibility};

	data_to_send = JSON.stringify(data_to_send);

	$("#box_reschedule_top").showLoading();
	$.ajax({
		url:site_url("tnpcell/allot_date_inf/RescheduleCompany/"),
		type:"post",
		data:data_to_send,
		dataType:"json",
		success:function(data){
...
function RemoveAllotedDate(company_id)
{
	$box_reschedule_top.showLoading();
	$.ajax({
		url:site_url("tnpcell/allot_date_inf/RemoveAllotedDate/"+company_id),
		success:function(data){
			//alert("Company Rescheduled successfully");
			$box_reschedule_top.hideLoading();
			document.location.href = "allot_date_inf";
		},
		type:"post",
		fail:function(error){
			console.log(error);
...
function change_status(company_id)
{
	...
	$.ajax({
		...
		//alert(status);
		url:site_url("tnpcell/allot_date_inf/ChangeStatus/"+company_id+"/"+status),
		success:function(data){
			// alert("Company Rescheduled successfully");
			// console.log(data);
