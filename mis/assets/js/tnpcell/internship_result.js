...
$("#ddl_add_result_course").change(function(){
	$box_form = $("#box_stu_result");
		...
		$.ajax({url:site_url("tnpcell/stu_internship/json_get_branch/"+$("#ddl_add_result_course").find(':selected').val()),
...
$("#ddl_add_result_branch").change(function(){
	$box_form = $("#box_stu_result");
		$box_form.showLoading();
		$.ajax({url:site_url("tnpcell/stu_internship/json_get_student_from_course_branch_for_internship/"+$("#ddl_add_result_course").find(':selected').val()+"/"+$
		("#ddl_add_result_branch").find(':selected').val()),
			success:function(data){
				console.log(data);	
				var base_str = "<option value = '' selected='selected' disabled>Select Name</option>";
				for($d=0 ; $d < data.length;$d++) {
...
$("#ddl_view_result_course").change(function(){
	$box_form = $("#box_result");
		$box_form.showLoading();
		$.ajax({url:site_url("tnpcell/stu_internship/json_get_branch/"+$("#ddl_view_result_course").find(':selected').val()),
			success:function(data){
				console.log("succeess");
