...
//echo form_open('complaint/register_complaint/insert');   
	...
	$form = $ui->form()->action('complaint/register_complaint/insert')->open();//calling function insert from complaint model named register_complaint on submit
	...
	$inputRow1 = $ui->row()->open();
		...
		$ui->select()
		   ...
		   ->required()
		   ->options(array( $ui->option()->value('""')->text('Select'),
							$ui->option()->value("NA")->text('NA for Stu Contingency'), 
							...
							$ui->option()->value("SAH")->text('SAH'),
							$ui->option()->value("EDC")->text('EDC'),
