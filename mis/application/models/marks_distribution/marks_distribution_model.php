...
function checkforGroupingByCode($data){
  ...
  $sql="select * from cbcs_optional_mapping_desc a
        inner join cbcs_optional_mapping b on a.map_id=b.id and b.session_year='$session_year' and b.`session`='$session'
        where a.emp_no='$emp_no' and a.sub_id='$sub_code'";
...
function checkforGrouping($data){
  $emp_no=$data['emp_id'];
  $session_yr=$data['session_year'];
  $session=$data['session'];
  $sql="select * from cbcs_optional_mapping_desc a
        inner join cbcs_optional_mapping b on a.map_id=b.id and b.session_year='$session_yr' and b.`session`='$session'
        where a.emp_no='$emp_no' AND a.coordinator=1";
        $query = $this->db->query($sql);
      //  echo $this->db->last_query(); die();
        if ($query->num_rows() > 0)
            return $query->result();
...
function getGroupingSubjects($data,$type){
  if($type=="jrf"){
   $extraParam="and b.course_id='jrf'";
 }else{
   $extraParam="and b.course_id<>'jrf'";
 }
  $emp_no=$data['emp_id'];
  $session_yr=$data['session_year'];
  $session=$data['session'];
 /* $sql="SELECT newt.sub_offer_id AS map_id, newt.semester, newt.sub_name AS sub_name, newt.sub_id, newt.branch_id,
newt.group, newt.section, newt.branch_name, newt.course_id,newt.sub_category,newt.aggr_id, newt.course_name, newt.coordinator,
newt.sub_type,true as 'isgrouping',newt.count_from,newt.count_to
FROM (
select  a.map_id as group_id,a.emp_no,a.section,a.sub_id,a.part,b.id,b.session_year,b.`session`,b.sub_offered_id as sub_offer_id,
b.sub_code,d.sub_name,d.sub_type,d.semester,d.sub_group as `group`,b.sub_category,b.course_id,b.branch_id,b.group_no,b.count_from,
b.count_to,c.sub_offered_id,a.coordinator,(case when b.sub_category like 'ESO%' then group_concat(b.branch_id) else cs_branches.name end) AS branch_name,cs_courses.name AS course_name,'cbcs' AS aggr_id from cbcs_optional_mapping_desc a
inner join cbcs_optional_mapping b on a.map_id=b.id and b.session_year='$session_yr' and b.`session`='$session'
inner join cbcs_subject_offered_desc c on a.emp_no=c.emp_no and a.sub_id=c.sub_id and SUBSTRING(b.sub_offered_id,2)=c.sub_offered_id 
and (case when a.coordinator=0 and c.coordinator=1 then a.coordinator=1 else 1=1 end) 
inner join cbcs_subject_offered d on c.sub_offered_id=d.id
LEFT JOIN cs_branches ON b.branch_id=cs_branches.id
LEFT JOIN cs_courses ON b.course_id=cs_courses.id
and b.sub_offered_id=concat('c',c.sub_offered_id)
where a.emp_no='$emp_no' $extraParam group by (case when b.sub_category like 'ESO%' then b.sub_offered_id else b.sub_offered_id end)
union
select  a.map_id as group_id,a.emp_no,a.section,a.sub_id,a.part,b.id,b.session_year,b.`session`,b.sub_offered_id as sub_offer_id,
b.sub_code,d.sub_name,d.sub_type,d.semester,d.sub_group as `group`,b.sub_category,b.course_id,b.branch_id,b.group_no,b.count_from,
b.count_to,c.sub_offered_id,a.coordinator,(case when b.sub_category like 'ESO%' then group_concat(b.branch_id) else cs_branches.name end) AS branch_name,cs_courses.name AS course_name,'cbcs' AS aggr_id from cbcs_optional_mapping_desc a
inner join cbcs_optional_mapping b on a.map_id=b.id and b.session_year='$session_yr' and b.`session`='$session'
inner join old_subject_offered_desc c on a.emp_no=c.emp_no and a.sub_id=c.sub_id and SUBSTRING(b.sub_offered_id,2)=c.sub_offered_id 
and (case when a.coordinator=0 and c.coordinator=1 then a.coordinator=1 else 1=1 end) 
inner join old_subject_offered d on c.sub_offered_id=d.id
LEFT JOIN cs_branches ON b.branch_id=cs_branches.id
LEFT JOIN cs_courses ON b.course_id=cs_courses.id
and b.sub_offered_id=concat('o',c.sub_offered_id)
where a.emp_no='$emp_no' $extraParam group by (case when b.sub_category like 'ESO%' then b.sub_offered_id else b.sub_offered_id end)
)newt"; */


$sql="
SELECT newt.sub_offer_id AS map_id, newt.semester, newt.sub_name AS sub_name, newt.sub_id, newt.branch_id, newt.group, newt.section, newt.branch_name, newt.course_id,newt.sub_category,newt.aggr_id, newt.course_name, newt.coordinator, newt.sub_type, TRUE AS 'isgrouping',newt.count_from,newt.count_to
FROM (
SELECT a.map_id AS group_id,a.emp_no,a.section,a.sub_id,a.part,b.id,b.session_year,b.`session`,b.sub_offered_id AS sub_offer_id, b.sub_code,d.sub_name,d.sub_type,d.semester,d.sub_group AS `group`,b.sub_category,b.course_id,b.branch_id,b.group_no,b.count_from, b.count_to,c.sub_offered_id,c.coordinator,(CASE WHEN b.sub_category LIKE 'ESO%' THEN GROUP_CONCAT(b.branch_id) ELSE cs_branches.name END) AS branch_name,cs_courses.name AS course_name,'cbcs' AS aggr_id
FROM cbcs_optional_mapping_desc a
INNER JOIN cbcs_optional_mapping b ON a.map_id=b.id AND b.session_year='$session_yr' AND b.`session`='$session'
LEFT JOIN cbcs_subject_offered_desc c ON a.emp_no=c.emp_no AND a.sub_id=c.sub_id AND SUBSTRING(b.sub_offered_id,2)=c.sub_offered_id 

AND (CASE WHEN a.coordinator=0 AND c.coordinator=1 THEN a.coordinator=1 ELSE 1=1 END)
LEFT JOIN cbcs_subject_offered d ON c.sub_offered_id=d.id
LEFT JOIN cs_branches ON b.branch_id=cs_branches.id
LEFT JOIN cs_courses ON b.course_id=cs_courses.id AND b.sub_offered_id= CONCAT('c',c.sub_offered_id)
WHERE a.emp_no='$emp_no' $extraParam
GROUP BY (CASE WHEN b.sub_category LIKE 'ESO%' THEN b.sub_offered_id ELSE b.id END) 
UNION
SELECT a.map_id AS group_id,a.emp_no,a.section,a.sub_id,a.part,b.id,b.session_year,b.`session`,b.sub_offered_id AS sub_offer_id, b.sub_code,d.sub_name,d.sub_type,d.semester,d.sub_group AS `group`,b.sub_category,b.course_id,b.branch_id,b.group_no,b.count_from, b.count_to,c.sub_offered_id,c.coordinator,(CASE WHEN b.sub_category LIKE 'ESO%' THEN GROUP_CONCAT(b.branch_id) ELSE cs_branches.name END) AS branch_name,cs_courses.name AS course_name,'cbcs' AS aggr_id
FROM cbcs_optional_mapping_desc a
INNER JOIN cbcs_optional_mapping b ON a.map_id=b.id AND b.session_year='$session_yr' AND b.`session`='$session'
LEFT JOIN old_subject_offered_desc c ON a.emp_no=c.emp_no AND a.sub_id=c.sub_id AND SUBSTRING(b.sub_offered_id,2)=c.sub_offered_id 

LEFT JOIN old_subject_offered d ON c.sub_offered_id=d.id
LEFT JOIN cs_branches ON b.branch_id=cs_branches.id
LEFT JOIN cs_courses ON b.course_id=cs_courses.id AND b.sub_offered_id= CONCAT('o',c.sub_offered_id)
WHERE a.emp_no='$emp_no' $extraParam
GROUP BY (CASE WHEN b.sub_category LIKE 'ESO%' THEN b.sub_offered_id ELSE b.id END))newt
...
//echo"".$course_id;
    ...
    //  echo $this->db->last_query(); die();
      if ($query->num_rows() > 0)
	    ...
	    public function getSubjectPlanList($data,$checkforGrouping){
        //echo $checkforGrouping;exit;
      $session_year=  $data['primary']['session_year'];
      $session=  $data['primary']['session'];
     $branch_id=  $data['primary']['branch_id'];
      $jrfstatus=  $data['primary']['jrfstatus'];
...
//echo"".$course_id;
         ...
         where a.session_year='$session_year'
          ...
          //    echo "<pre>";print_r($data['primary']); exit;
		   ...
		   if($sub_category=='OE' || $sub_category=='DE'){
            ...
            INNER JOIN cbcs_marks_dist_child b ON a.id=b.id
			$extraJoin_cbcs
            inner JOIN cbcs_subject_offered c ON
            (case when c.sub_category like 'ESO%' then a.map_id=concat('c',c.id) else a.course_id=c.course_id end)
            AND (case when c.sub_category like 'ESO%' then a.map_id=concat('c',c.id) 
			when exists (SELECT * FROM cbcs_optional_mapping m WHERE a.sub_code=m.sub_code AND a.branch_id=m.branch_id) then a.map_id= CONCAT('c',c.id)

			else a.branch_id=c.branch_id end)
            AND a.sub_code=c.sub_code AND a.session_year=c.session_year  and a.session=c.session $semclouse $joinGroup
			inner join cbcs_subject_offered_desc d on c.id=d.sub_offered_id  $extraSec
		   WHERE a.session_year='$session_year' AND a.`session`='$session' AND (case when c.sub_category like 'ESO%' then 1=1 else a.branch_id='$branch_id' end)
       AND (case when c.sub_category like 'ESO%' then 1=1 else a.course_id='$course_id' end)
       AND (case when c.sub_category like 'ESO%' then 1=1 else a.dept_id='$dept_id' end)
       AND a.sub_code='$sub_code'
       AND (case when c.sub_category like 'ESO%' then 1=1 else a.semester='$semester' end) AND a.emp_no='$emp_no' $addgroup $commCouse
            ...
            INNER JOIN cbcs_marks_dist_child b ON a.id=b.id
			$extraJoin_old
            inner JOIN old_subject_offered c ON  (case when c.sub_category like 'ESO%' then a.map_id=concat('o',c.id) else a.course_id=c.course_id end)
            AND (case when c.sub_category like 'ESO%' then a.map_id=concat('o',c.id) 
			when exists (SELECT * FROM cbcs_optional_mapping m WHERE a.sub_code=m.sub_code AND a.branch_id=m.branch_id) then a.map_id= CONCAT('o',c.id)
  ...
  /* 8-11-19  public function getSubjectPlanList($data){
   ...
   function checkCourseCoordinator($session,$sessionyear,$sub_code,$sub_offerd_id,$sub_type){
      ...
      if($sub_type=='Modular' && (($sub_code!='ESI101' || $sub_code!='ECI102' ) && $session!='Winter' && $sessionyear=='2020-2021')){
...
and a.subject_code=? and b.uploaded_by=? group by a.admn_no";*/
 /* 04-02-2020 $sql="select a.*, GROUP_CONCAT(concat_ws(',',b.category_id)) as marks,GROUP_CONCAT(b.marks) as total,concat_ws(' ',c.salutation,c.first_name,c.middle_name,c.last_name) as name
  ...
  and (case when d.sub_category like 'DE%' OR d.sub_category like 'OE%' OR a.branch_id='Online' OR d.sub_category like 'ESO%' then 1=1 else a.branch_id='$branch_id' end)
  and (case when d.sub_category like 'DE%' OR d.sub_category like 'OE%' OR a.branch_id='Online' OR d.sub_category like 'ESO%' then 1=1 else a.course_id='$course_id' end)
...
and a.subject_code=? and b.uploaded_by=? group by a.admn_no";*/
 /* $sql="select a.*, GROUP_CONCAT(concat_ws(',',b.category_id)) as marks,GROUP_CONCAT(b.marks) as total,concat_ws(' ',c.first_name,c.middle_name,c.last_name) as name
  ...
  where a.session_year=? and a.`session`=?
  AND (case when d.sub_category like 'OE%' OR d.sub_category like 'DE%' OR d.sub_category LIKE 'ESO%' then 1=1 else a.branch_id='$branch_id' end )
  AND (case when d.sub_category like 'OE%' OR d.sub_category like 'DE%' OR d.sub_category LIKE 'ESO%' then 1=1 else a.course_id='$course_id' end )
...
public function getSubjectPlanforview($sessionyear,$session,$branch_id,$course_id,$sub_code,$extraParam,$isGrouping){
  ...
  if($isGrouping){
    $groupingclouse="and a.emp_no='$emp_id'";
    $normal="";
  }else{
    $groupingclouse="";
    $normal="AND a.branch_id='$branch_id' AND a.course_id='$course_id'";
  }
...
WHERE a.session_year='$sessionyear' AND a.`session`='$session' $normal AND a.sub_code='$sub_code' and a.id='$main_marks_id' $groupingclouse $addgroup order by b.category asc";
  $query = $this->db->query($sql);
  //echo $this->db->last_query(); die();
  if ($query->num_rows() > 0)
      return $query->result();
...
public function getGrouingDetails($data){
  $session_year=  $data['primary']['session_year'];
  $session=  $data['primary']['session'];
  $sub_code=  $data['primary']['sub_code'];
  $sub_offerd_id=  $data['primary']['map_id'];
  $sub_code=  $data['primary']['sub_code'];
  $emp_no=  $data['primary']['emp_no'];
  $branch_id=  $data['primary']['branch_id'];
 /* $sql="SELECT a.*,b.*
FROM cbcs_optional_mapping a
INNER JOIN cbcs_optional_mapping_desc b ON a.id=b.map_id
INNER join old_subject_offered c on a.sub_code=c.sub_code and (case when a.sub_category like 'ESO%' then 1=1 else substring(a.sub_offered_id,2)='$sub_offerd_id' end)
and a.session_year=c.session_year and a.`session`=c.`session`and
(case when a.sub_category like 'ESO%' then 1=1 else a.branch_id=c.branch_id end) and
(case when a.sub_category like 'ESO%' then 1=1 else a.course_id=c.course_id end)
WHERE a.session_year='$session_year' AND a.`session`='$session' AND a.sub_code='$sub_code' AND b.emp_no='$emp_no'
union
SELECT a.*,b.*
FROM cbcs_optional_mapping a
INNER JOIN cbcs_optional_mapping_desc b ON a.id=b.map_id
INNER join cbcs_subject_offered c on a.sub_code=c.sub_code and (case when a.sub_category like 'ESO%' then 1=1 else substring(a.sub_offered_id,2)='$sub_offerd_id' end)
and a.session_year=c.session_year and a.session=c.session and
(case when a.sub_category like 'ESO%' then 1=1 else a.branch_id=c.branch_id end) and
(case when a.sub_category like 'ESO%' then 1=1 else a.course_id=c.course_id end)
WHERE a.session_year='$session_year' AND a.`session`='$session' AND a.sub_code='$sub_code' AND b.emp_no='$emp_no'"; */
...
FROM cbcs_optional_mapping a
INNER JOIN cbcs_optional_mapping_desc b ON a.id=b.map_id
WHERE a.session_year='$session_year' AND a.`session`='$session' AND a.sub_code='$sub_code' AND b.emp_no='$emp_no' AND a.sub_offered_id='$sub_offerd_id' AND a.branch_id='$branch_id'
...
public function getStudentListwithMarks($data){
      ...
      $sub_type=  $data['primary']['sub_type'];
      $count_from=$data['count_from'];
      $count_to=$data['count_to'];//die();
		$comm="";
		$sections="";
		$news="";
      if($branch_id=='comm' && $course_id=='comm'){
      ...
      if($sub_type=='Modular'){
        $innerjoin="INNER JOIN cbcs_modular_paper_details d on x.admn_no=d.admn_no /*and x.form_id=d.form_id*/ and d.$modular_exam_type in('$sub_code')";
        $condition="group by x.form_id";
      }else{
	     $innerjoin="";
        $condition="";
  ...
  //    echo "sss".$data['isGrouping'];exit;
if($data['isGrouping'] !='1'){
...
INNER JOIN user_details c  ON trim(a.admn_no)=trim(c.id)
LEFT JOIN cbcs_marks_upload mu on a.admn_no=mu.admn_no and a.session_year=mu.session_year and a.session=mu.session and a.subject_code=mu.subject_code
Left JOIN cbcs_marks_upload_description mud on mud.marks_id=mu.id and mud.category_id='$dist_id'
...
(case when a.sub_category like 'OE%' OR a.sub_category like 'DE%' OR a.sub_category like 'ESO%' OR a.branch='Online' then 1=1 else a.course='$course_id' end) and
(case when a.sub_category like 'OE%' OR a.sub_category like 'DE%' OR a.sub_category like 'ESO%' OR a.branch='Online' then 1=1 else a.branch='$branch_id' end)
...
and a.session_year='$session_year' and a.`session`='$session' $comm ) x
$innerjoin
...
inner join users uu on x.admn_no=uu.id and uu.status='A' $condition
";
}else{
  $sql="(SELECT a.form_id,a.admn_no,a.session_year,a.`session`,a.branch,a.course, CONCAT_WS(' ',c.first_name,c.middle_name,c.last_name) AS name,a.subject_code,a.subject_name,mud.marks
FROM cbcs_stu_course a
INNER JOIN reg_regular_form b ON b.form_id=a.form_id AND b.admn_no=a.admn_no and b.session_year='$session_year' and b.`session`='$session' and b.hod_status='1' and b.acad_status='1'
INNER JOIN user_details c ON trim(c.id)=trim(b.admn_no)
INNER JOIN users d ON d.id=c.id AND d.`status`='A'
LEFT JOIN cbcs_marks_upload mu ON a.admn_no=mu.admn_no AND a.session_year=mu.session_year AND a.session=mu.session AND a.subject_code=mu.subject_code
LEFT JOIN cbcs_marks_upload_description mud ON mud.marks_id=mu.id AND mud.category_id='$dist_id'
WHERE b.session_year='$session_year' AND b.`session`='$session' AND a.subject_code='$sub_code'
AND b.hod_status='1' AND b.acad_status='1'
AND b.course_id='$course_id' AND b.branch_id='$branch_id' AND a.sub_offered_id='$sub_offerd_id'
ORDER BY c.id  LIMIT $count_from,$count_to)
union
(SELECT a.form_id,a.admn_no,a.session_year,a.`session`,a.branch,a.course, CONCAT_WS(' ',c.first_name,c.middle_name,c.last_name) AS name,a.subject_code,a.subject_name,mud.marks
FROM old_stu_course a
INNER JOIN reg_regular_form b ON b.form_id=a.form_id AND b.admn_no=a.admn_no AND b.admn_no=a.admn_no and b.session_year='$session_year' and b.`session`='$session' and b.hod_status='1' and b.acad_status='1'
INNER JOIN user_details c ON trim(c.id)=trim(b.admn_no)
INNER JOIN users d ON d.id=c.id AND d.`status`='A'
LEFT JOIN cbcs_marks_upload mu ON a.admn_no=mu.admn_no AND a.session_year=mu.session_year AND a.session=mu.session AND a.subject_code=mu.subject_code
LEFT JOIN cbcs_marks_upload_description mud ON mud.marks_id=mu.id AND mud.category_id='$dist_id'
...
WHERE a.sub_code='$sub_code' AND a.`session`='$session' AND a.session_year='$session_year' AND b.emp_no='$emp_no')
  ...
  //  echo  $this->db->last_query(); die();
    ...
    function get_md_id($emp_id, $course_id, $branch_id, $sem, $sub_code, $section,$session_year,$session, $param = null) {
      ...
      //if
        ...
        if ($param <> null)
            $add2 = "  and  a.lecture_plan_path is not null ";
        $add = null;
        if ($course_id == 'comm')
            $add = " and  a.section = '$section' ";
      // 01/12/2020  $sql = " select id from cbcs_marks_dist WHERE emp_no='$emp_id'   and   course_id='$course_id' and branch_id='$branch_id' and semester='$sem' and  sub_code='$sub_code'  and session='$session' and session_year='$session_year' $add    $add2 ";


        $sql="
        SELECT a.id
        FROM cbcs_marks_dist a
        inner join cbcs_subject_offered b on a.sub_code=b.sub_code and a.session_year=b.session_year and a.session=b.session
        and (case when b.sub_category like 'ESO%' then a.map_id=concat('c',b.id) else 1=1 end) and (case when b.sub_category like 'ESO%' then 1=1 else a.course_id=b.course_id end)
        and (case when b.sub_category like 'ESO%' then 1=1 when exists (SELECT * FROM cbcs_optional_mapping m WHERE a.sub_code=m.sub_code AND a.branch_id=m.branch_id) then 1=1 else a.branch_id=b.branch_id end)
        WHERE a.emp_no='$emp_id' and (case when b.sub_category like 'ESO%' then 1=1 else a.course_id='$course_id' end) and (case when b.sub_category like 'ESO%' then 1=1 else a.branch_id='$branch_id' end) and (case when b.sub_category like 'ESO%' then 1=1 else  a.semester='$sem' end) and  a.sub_code='$sub_code'  and a.session='$session' and a.session_year='$session_year' $add    $add2
    
		    
		union
        SELECT a.id
        FROM cbcs_marks_dist a
        inner join old_subject_offered b on a.sub_code=b.sub_code and a.session_year=b.session_year and a.session=b.session
        and (case when b.sub_category like 'ESO%' then a.map_id=concat('o',b.id) else 1=1 end) and (case when b.sub_category like 'ESO%' then 1=1 else a.course_id=b.course_id end)
        and (case when b.sub_category like 'ESO%' then 1=1 
		when exists (SELECT * FROM cbcs_optional_mapping m WHERE a.sub_code=m.sub_code AND a.branch_id=m.branch_id) then 1=1
		else a.branch_id=b.branch_id end)
        WHERE a.emp_no='$emp_id'  and (case when b.sub_category like 'ESO%' then 1=1 else a.course_id='$course_id' end) and (case when b.sub_category like 'ESO%' then 1=1 else a.branch_id='$branch_id' end)
		    and (case when b.sub_category like 'ESO%' then 1=1 else  a.semester='$sem' end) and  a.sub_code='$sub_code'  and a.session='$session' and a.session_year='$session_year' $add    $add2
        ";
  ...
  function get_modular_before($session_yr,$session, $sub_code, $section){
	 // echo"section". $section;
	  if(!isset($section) || $section=="na" || $section==null){
		   $sql = " select count(id) as ctr from cbcs_modular_paper_details WHERE session=? and  session_year=? and   before_mid=?";
        $result = $this->db->query($sql,array($session,$session_yr,$sub_code));
	    ...
	    function get_modular_after($session_yr,$session, $sub_code, $section){
			 if(!isset($section) || $section=="na" || $section==null){ 
				  $sql = " select count(id) as ctr from cbcs_modular_paper_details WHERE session=? and  session_year=?  and   after_mid=?";
        $result = $this->db->query($sql,array($session,$session_yr, $sub_code));
        return $result->row()->ctr;
			 }else{
	...
	public function get_subjects($data,$Grouping ,$param = true) {
        ...
        if($Grouping){
          $gclouse="";
          $gjoin="INNER JOIN cbcs_optional_mapping com on A.sub_id=com.sub_code and A.sub_offered_id=SUBSTRING(com.sub_offered_id,2)
          inner join cbcs_optional_mapping_desc comd on com.id=comd.map_id and com.sub_code=comd.sub_id and comd.emp_no='$emp_id'";
        }else{
          $gclouse="and  newt.coordinator='1'";
          $gjoin="";
        }
        ...
        $cbcs_sys_sql = "  SELECT  concat('o',newt.id) AS map_id,
                        ...
                        WHERE newt.emp_no = '" . $emp_id . "'   $gclouse  AND SESSION='" . $session . "' AND session_year='" . $session_year . "' AND newt.course_id<>'jrf' AND  (newt.sub_category NOT like 'OE%' AND newt.sub_category NOT like  'DE%' ) $cbcs_rep1
        ...
        // end
	      ...
	      public function get_subjects_DE_OE($data,$param = true,$jf,$Grouping) {
		//  echo $jf;exit;

			if($jf=='1'){
				$withjrf="AND newt.course_id='jrf' and newt.sub_id not like '%599'";
			}else{
           ...
           $cbcs_sys_sql = "  SELECT  concat('o',newt.sub_offered_id)  AS map_id,
                           ...
                           WHERE newt.emp_no = '" . $emp_id . "'   $gclouse  AND SESSION='" . $session . "' AND session_year='" . $session_year . "'
                          $withjrf AND (newt.sub_category LIKE 'OE%' OR newt.sub_category LIKE 'DE%') 
                           ...
                           WHERE newt.emp_no = '" . $emp_id . "'   $gclouse  AND SESSION='" . $session . "' AND session_year='" . $session_year . "' $withjrf AND  (newt.sub_category  like 'OE%' OR newt.sub_category  like  'DE%' ) 
   ...
   public function get_subjects_jrf($data, $param = true,$Grouping) {
...
AND  (newt.sub_category NOT like 'OE%' AND newt.sub_category NOT like  'DE%' ) and newt.sub_id not like '%599'
$cbcs_rep1
		                     UNION
                         SELECT  concat('c',newt.id) AS map_id,
                                newt.semester,
  ...
  /*  public function get_subjects_jrf($data, $param = true) {
    ...
    public function get_prac_subjects($data,$param=null,$Grouping) {
		...
		$cbcs_sys_sql = "
		                 SELECT concat('o',newt.id) AS map_id, newt.semester, newt.sub_name AS sub_name, newt.sub_id, newt.branch_id, newt.group, newt.section, newt.branch_name, newt.course_id, newt.aggr_id, newt.course_name, newt.coordinator, newt.sub_type
                        FROM (
                             SELECT B.id,B.SESSION,B.session_year,B.sub_code AS sub_id, B.branch_id,B.course_id,B.sub_group AS `group`, B.semester, A.emp_no, A.sub_offered_id AS map_id,cs_branches.name AS branch_name,cs_courses.name AS course_name,'old' AS aggr_id, B.sub_name,A.coordinator,B.sub_type,A.section
                              FROM old_subject_offered_desc AS A
                              INNER JOIN old_subject_offered AS B ON A.sub_offered_id = B.id
							  JOIN old_stu_course osc1 ON  osc1.sub_offered_id=B.id
                              ...
                              LEFT JOIN cs_branches ON B.branch_id=cs_branches.id
                              LEFT JOIN cs_courses ON B.course_id=cs_courses.id) AS newt
                              WHERE newt.emp_no = '" . $emp_id . "'  $gclouse   AND SESSION='" . $session . "' AND session_year='" . $session_year . "'   AND newt.course_id<>'jrf' $cbcs_rep1
                     		   UNION
                               SELECT  concat('c',newt.id) AS map_id,
                                newt.semester,
                                newt.sub_name AS sub_name,
    ...
    //==========================================JRF==============================================
     ...
     public function get_prac_subjects_jrf($data,$param=null,$Grouping) {
        $emp_id = $data['emp_id'];
        $session_year = $data['session_year'];
        $session = $data['session'];
        $this->load->database();
        ...
        LEFT JOIN cs_branches ON B.branch_id=cs_branches.id
        LEFT JOIN cs_courses ON B.course_id=cs_courses.id) AS newt
        WHERE newt.emp_no = '" . $emp_id . "' $gclouse AND SESSION='" . $session . "' AND session_year='" . $session_year . "'  AND newt.course_id='jrf' $cbcs_rep1 and newt.sub_id not like '%599'
        UNION
