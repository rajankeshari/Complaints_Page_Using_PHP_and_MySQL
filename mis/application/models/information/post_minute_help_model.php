...
class Post_minute_help_model extends CI_Model
{
	...
	public function delete_group_global($minutes_id)
	{
		
		$query="DELETE from minutes_group_global_id where minutes_id='$minutes_id'";
		$this->db->query($query);
	}
	...
	function get_group_global($data)
	{
		...
		$minutes_id=$data['minutes_id'];
		$created_by=$data['id_user'];
		$query = "INSERT INTO minutes_group_global_id VALUES ('$minutes_id','$temp')";
		$this->db->query($query);
	}
