...
class Viewcircular_model extends CI_Model
{
	...
	function get_circular_ids()
	{
		...
		$query=("SELECT circular_id
			FROM info_circular_archieve_details
			...
			UNION
			SELECT  circular_group_global_id.circular_id
			FROM info_group_global
			INNER JOIN circular_group_global_id 
			ON info_group_global.group_id=circular_group_global_id.group_id 
			...
			ORDER BY info_circular_archieve_details.posted_on desc");
	...
	function get_circulars()
	{
		...
		else if($notice_cat=='emp')
		{
			...
			$query=("SELECT info_circular_archieve_details.*, user_details.*, auth_types.type as auth_name,
			 departments.name as department, designations.name as designation
			FROM info_circular_archieve_details
			INNER JOIN user_details 
			 ON info_circular_archieve_details.issued_by=user_details.id
			INNER JOIN auth_types 
			 ON info_circular_archieve_details.auth_id=auth_types.id
