...
class Viewminute_model extends CI_Model
{
	...
	function get_minute_ids()
	{
		...
		else if($meeting_cat=='emp')
		{
			...
			FROM info_minute_archieve_details
			INNER JOIN user_details 
			 ON info_minute_archieve_details.issued_by=user_details.id
			INNER JOIN auth_types 
			 ON info_minute_archieve_details.auth_id=auth_types.id
			...
			UNION 
			SELECT  minutes_group_global_id.minutes_id
			FROM info_group_global
			INNER JOIN minutes_group_global_id 
			ON info_group_global.group_id=minutes_group_global_id.group_id 
			...
			ORDER BY info_minute_archieve_details.posted_on desc");
	...
	function get_minutes()
	{
		...
		else if($meeting_cat=='emp')
		{
			...
			$query=("SELECT info_minute_archieve_details.*, user_details.*, auth_types.type as auth_name,
