...
class Online_crf_booking_model extends CI_Model {
	...
	public function get_booking_detail($id){ 
		$sql="SELECT cics.detail_id,ci.inst_id,cif.inst_fac_id,ci.email,uod.mobile_no,ci.fic_emp_no,ci.instrument,cif.facility,cics.analysis_charge,cics.sample_per_day,cics.sample_per_night,cics.slot_allotment,cics.slot_per_day,cics.slot_per_night FROM crf_inst_facility cif INNER JOIN crf_inst ci ON cif.inst_id=ci.inst_id INNER JOIN crf_inst_cost_and_slot cics ON ci.inst_id=cics.inst_id AND cif.inst_fac_id=cics.inst_fac_id INNER JOIN user_other_details uod ON uod.id=ci.fic_emp_no WHERE cif.inst_fac_id='$id'";
	...
	public function get_basic_detail($id){
		...
		if($cquery->num_rows() >0){
			...
			if($auth_id=="emp"){
				$sql="SELECT ud.id,ud.salutation,ud.first_name,if(ud.middle_name=' ','NA',ud.middle_name) AS mname,ud.last_name,cd.name AS dept,dg.name AS idesg,ee.domain_name AS email,uod.mobile_no AS mobile,auth.type AS user FROM users u INNER JOIN user_details ud ON u.id=ud.id INNER JOIN cbcs_departments cd ON ud.dept_id=cd.id LEFT JOIN emp_basic_details emp ON emp.emp_no=u.id LEFT JOIN designations dg ON dg.id=emp.designation INNER JOIN auth_types auth ON auth.id=emp.auth_id LEFT JOIN emaildata_emp ee ON u.id=ee.emp_id INNER JOIN user_other_details uod ON uod.id=u.id WHERE u.id='$id' AND u.`status`='A' AND auth.id='ft'";
	...
	public function add_pay_data($name){
		...
		$this->db->trans_start();
		$this->db->insert('crf_booking_user',$insert_data);
			...
			if(empty($name['pay_date'])){
				...
				$this->db->insert('crf_booking_payment',$insert_payment);
	...
	public function check_point($dates,$admn_no,$analysis_charge){

		if($analysis_charge==='Sample Based'){
			$sql = "SELECT SUM(cbsnc.day_sample) AS check_point,cbsnc.* FROM crf_booking_user cbu INNER JOIN crf_booking_s_n_c cbsnc ON cbu.user_id=cbsnc.user_id	WHERE cbu.int_id='$admn_no' AND cbsnc.dates='$dates' AND cbsnc.slot_id IS NULL";
			$query = $this->db->query($sql);
			// echo $this->db->last_query();
			if($query->num_rows()>0){
				$check_point= (int)$query->row()->check_point;
				if($check_point<5){
					return true;
				}
				else{
			...
			$sql = "SELECT COUNT(cbsnc.slot_id) AS check_point,cbsnc.* FROM crf_booking_user cbu INNER JOIN crf_booking_s_n_c cbsnc ON cbu.user_id=cbsnc.user_id	WHERE cbu.int_id='$admn_no' AND cbsnc.dates='$dates' AND cbsnc.slot_id IS NOT NULL";
			$query = $this->db->query($sql);
			// echo $this->db->last_query();
			if($query->num_rows()>0){
				$check_point= (int)$query->row()->check_point;
	...
	public function check_point_number($dates,$admn_no,$analysis_charge){
		if($analysis_charge==='Sample Based'){
			$sql = "SELECT if(cbsnc.slot_id IS NULL,SUM(cbsnc.day_sample+cbsnc.night_sample),COUNT(cbsnc.slot_id)) AS check_point,cbsnc.* FROM crf_booking_user cbu INNER JOIN crf_booking_s_n_c cbsnc ON cbu.user_id=cbsnc.user_id	WHERE cbu.int_id='$admn_no' AND cbsnc.dates='$dates' AND cbsnc.slot_id IS NULL";
			$query = $this->db->query($sql);
			// echo $this->db->last_query();
			if($query->num_rows()>0){
				$check_point= (int)$query->row()->check_point;
			...
			$sql = "SELECT if(cbsnc.slot_id IS NULL,SUM(cbsnc.day_sample+cbsnc.night_sample),COUNT(cbsnc.slot_id)) AS check_point,cbsnc.* FROM crf_booking_user cbu INNER JOIN crf_booking_s_n_c cbsnc ON cbu.user_id=cbsnc.user_id	WHERE cbu.int_id='$admn_no' AND cbsnc.dates='$dates' AND cbsnc.slot_id IS NOT NULL";
			$query = $this->db->query($sql);
			// echo $this->db->last_query();
			if($query->num_rows()>0){
				$check_point= (int)$query->row()->check_point;
	...
	public function get_sample_booked_detail(){
		...
		if(in_array('adri',$auth)){
			...
			$token = "cbu.int_id='".$empid."' AND ";

		}
		$sql = "SELECT cbu.booking_TS AS created_date,cbu.user_id,cbu.user_category,cbu.user_purpose,cbu.int_id,ci.instrument,cif.facility,
		(case when cbsnc.day_sample!=0 then 'Day Mode' ELSE 'Night Mode' END) AS modes,
		(case when cbsnc.day_sample!=0 then cbsnc.day_sample ELSE cbsnc.night_sample END) AS sample,
		cbsnc.charge,cbsnc.dates FROM crf_booking_user cbu
		INNER JOIN crf_booking_s_n_c cbsnc ON cbu.user_id=cbsnc.user_id
		LEFT JOIN crf_inst ci ON ci.inst_id=cbsnc.inst_id
		LEFT JOIN crf_inst_facility cif ON cif.inst_fac_id=cbsnc.inst_fac_id
		WHERE $token cbsnc.slot_id IS NULL
		ORDER BY cbu.booking_TS DESC";
	...
	public function get_hour_booked_detail(){
		$empid = $this->session->userdata('id');
		$auth = $this->session->userdata('auth');
		if(in_array('adri',$auth)){
			$token = '';
		...
		$sql = "SELECT cbu.booking_TS AS created_date,cbu.user_id,cbu.user_category,cbu.user_purpose,cbu.int_id,ci.instrument,cif.facility,
		(case when cbsnc.d_start_time IS NOT NULL OR cbsnc.d_end_time IS NOT NULL then 'Day Mode' ELSE 'Night Mode' END) AS modes,
		(case when cbsnc.d_start_time IS NOT NULL then cbsnc.d_start_time ELSE cbsnc.n_start_time END) AS day_slot,
		(case when cbsnc.d_end_time IS NOT NULL then cbsnc.d_end_time ELSE cbsnc.n_end_time END) AS night_slot,
		cbsnc.charge,cbsnc.dates FROM crf_booking_user cbu
		INNER JOIN crf_booking_s_n_c cbsnc ON cbu.user_id=cbsnc.user_id
		LEFT JOIN crf_inst ci ON ci.inst_id=cbsnc.inst_id
		LEFT JOIN crf_inst_facility cif ON cif.inst_fac_id=cbsnc.inst_fac_id
		WHERE $token cbsnc.slot_id IS NOT NULL
		ORDER BY cbu.booking_TS DESC";
