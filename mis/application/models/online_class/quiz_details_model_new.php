...
class Quiz_details_model_new extends CI_Model {   
	...
	function get_quiz_list($syear,$sess,$sub_code,$group_no,$section,$emp_id,$quiz_no){
			if($section=='A'||$section=='B'||$section=='C'||$section=='D'||$section=='E'||$section=='F'||$section=='G'||$section=='H'){
				 $sql="SELECT a.* FROM ft_online_quiz a WHERE a.session_year=? and a.session=? AND a.sub_code=?  and a.group_no=?  AND a.emp_no=? AND a.quiz_no=? and section=?";
              $query = $this->db->query($sql,array($syear,$sess,$sub_code,$group_no,$emp_id,$quiz_no,$section));
