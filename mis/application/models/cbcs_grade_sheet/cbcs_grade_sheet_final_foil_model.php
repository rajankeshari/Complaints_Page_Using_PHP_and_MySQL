...
function get_basic_detail($admn_no){
    ...
    ,u.id AS admn_no,if(cc.id='dualdegree',SUBSTRING_INDEX(cb.name, '+', 1),cb.name) AS bname,cc.id AS bid,cb.id AS cid FROM users u INNER JOIN user_details ud ON u.id=ud.id INNER JOIN stu_academic sa ON u.id=sa.admn_no LEFT JOIN cbcs_courses cc ON cc.id=sa.course_id LEFT JOIN cbcs_branches cb ON cb.id=sa.branch_id
...
function get_drop_detail($admn_no,$cid,$bid,$session_year,$session){
    $session_str =  ($session==='Monsoon' ? "and  session_year<='$session_year' and !(session_year='$session_year' and  (session='Monsoon' or session='Winter'  or session='Summer') )" : ( $session=='Winter' ?  " and  session_year<='$session_year' and !( session_year='$session_year' and ( session='Winter' or session='Summer') )" :($session=='Summer' ?  " and  session_year<='$session_year'   and !( session_year='$session_year' and ( session='Summer') )" : " and session_year<='$session_year' " )));
...
function get_grade_detail($admn_no){
    ...
    if (in_array("stu", $this->session->userdata('auth'))) {
        // $str = 'AND k.published_on IS NOT NULL';
        $str = 'AND a.published_on IS NOT NULL';
    }
	...
	if($cadmn_no=='14'){
      ...
      t.subje_name AS sub_name,t.crdhrs AS cr_hr,t.grade,ROUND(CAST(FORMAT(t.gpa,5) AS DECIMAL(10,2)),2) AS gpa,ROUND(CAST(FORMAT(t.ogpa,5) AS DECIMAL(10,2)),2) AS cgpa
	  ,t.totcrhr AS tot_cr_hr FROM tabulation1 t WHERE if(t.wsms='ZS',t.adm_no='$admn_no' AND t.subje_pf='SP',t.adm_no='$admn_no')
	  ...
	  /*$token = "SELECT s.session_year AS session_yr,s.session, GROUP_CONCAT(s.sub_code
ORDER BY s.sessions,s.sub_order SEPARATOR '@') AS sub_code, UCASE(GROUP_CONCAT(s.sub_name
ORDER BY s.sessions,s.sub_order SEPARATOR '@')) AS sub_name, GROUP_CONCAT(s.cr_hr
ORDER BY s.sessions,s.sub_order SEPARATOR '@') AS credit, GROUP_CONCAT(s.grade
ORDER BY s.sessions,s.sub_order SEPARATOR '@') AS grade,s.gpa,s.cgpa,s.tot_cr_hr
FROM (SELECT if(t.ysession='1415','2014-2015', NULL) AS session_year,
UCASE(CASE t.wsms WHEN 'MS' THEN 'Monsoon' WHEN 'WS' THEN 'Winter' ELSE 'Summer' END) AS SESSION,
(CASE WHEN t.wsms='MS' THEN 1 WHEN t.wsms='WS' THEN 2 ELSE 3 END) AS sessions,t.subje_code AS sub_code,
t.subje_order AS sub_order,t.subje_name AS sub_name,t.crdhrs AS cr_hr,t.grade,ROUND(CAST(FORMAT(t.gpa,5) AS DECIMAL(10,2)),2) AS gpa, ROUND(CAST(FORMAT(t.ogpa,5) AS DECIMAL(10,2)),2) AS cgpa,t.totcrhr AS tot_cr_hr FROM tabulation1 t WHERE if(t.wsms='ZS',t.adm_no='$admn_no' AND t.subje_pf='SP',t.adm_no='$admn_no')
ORDER BY t.ysession,t.wsms,Case WHEN t.wsms= 'ZS' THEN t.sem_code END DESC, Case WHEN 1=1 then t.sem_code END LIMIT 100000)s
GROUP BY  s.session_year,s.SESSION
ORDER BY s.sessions";*/
	  
	  $token = "SELECT s.session_year AS session_yr,s.session,
GROUP_CONCAT(s.sub_code ORDER BY s.sessions,s.sub_order SEPARATOR '@') AS sub_code,
 UCASE(GROUP_CONCAT(s.sub_name ORDER BY s.sessions,s.sub_order SEPARATOR '@')) AS sub_name,
 GROUP_CONCAT(s.cr_hr ORDER BY s.sessions,s.sub_order SEPARATOR '@') AS credit,
 GROUP_CONCAT(s.grade ORDER BY s.sessions,s.sub_order SEPARATOR '@') AS grade,s.gpa,s.cgpa,s.tot_cr_hr
FROM (
SELECT if(t.ysession='1415','2014-2015', NULL) AS session_year, t.ysession,t.wsms,t.sem_code,
UCASE(CASE t.wsms WHEN 'MS' THEN 'Monsoon' WHEN 'WS' THEN 'Winter' ELSE 'Summer' END) AS SESSION,
(CASE WHEN t.wsms='MS' THEN 1 WHEN t.wsms='WS' THEN 2 ELSE 3 END) AS sessions,t.subje_code AS sub_code,
t.subje_order AS sub_order,
 t.subje_name AS sub_name,t.crdhrs AS cr_hr,t.grade,
  ROUND(CAST(FORMAT(t.gpa,5) AS DECIMAL(10,2)),2) AS gpa, ROUND(CAST(FORMAT(t.ogpa,5) AS DECIMAL(10,2)),2) AS cgpa,
  t.totcrhr AS tot_cr_hr
FROM tabulation1 t
WHERE if(t.wsms='ZS',t.adm_no='$admn_no' AND t.subje_pf='SP',t.adm_no='$admn_no')
ORDER BY /*t.ysession,*/t.wsms/*,Case WHEN t.wsms= 'ZS' THEN t.sem_code END DESC, Case WHEN 1=1 then t.sem_code END*/
DESC LIMIT 100000)s
GROUP BY   s.ysession,s.wsms
ORDER BY  s.ysession,s.wsms";
...
$str  AND (if(a.course='JRF',a.semester>=0,(a.semester<>0 AND a.semester<>-1))) AND a.exam_type<>'S'
...
AND (if(a.course='JRF',a.semester>=0,(a.semester<>0 AND a.semester<>-1))) AND a.exam_type='S'
...
)s GROUP BY s.session_yr,s.session ORDER BY s.exam_type,s.session_yr,sessions,types";*/
...
public function check_monsoon($data)
{  

    $m="2021-2022";
    $stmt="SELECT g.admn_no FROM reg_regular_form g WHERE g.session_year='".$m."' AND g.session='Monsoon' AND g.admn_no='".$data['adm_no']."' ";
...
public function check_summer($data)
{  
    
    $stmt="SELECT g.* FROM reg_regular_form g WHERE g.session_year='".$data['year_']."' AND g.`session`='Summer' AND g.admn_no='".$data['adm_no']."' ";
    $query = $this->db->query($stmt);
    // echo $this->db->last_query();
    // exit;
   if($query->num_rows() > 0)
...
public function check_cgpa($data)
{  
    $s="2020-2021";
    $stmt="select p.cgpa,p.* from final_semwise_marks_foil_freezed p where p.session_yr='".$data['year_']."' 
    and p.`session`='".$data['session_']."' and p.dept='".$data['dept_id']."' and p.course='".$data['course_id']."' and
    p.branch='".$data['branch_id']."' and p.semester='".$data['semester']."' and p.cgpa>5 and p.admn_no='".$data['adm_no']."' ";
    $query = $this->db->query($stmt);
    // echo $this->db->last_query();
    // exit;
   if($query->num_rows() > 0)
