...
class Add_complaint extends CI_Model
{
	var $table = 'new_complaint';
	...
	/*function insert($data)
	{
        ...
        function insert($data)
        {
            if($this->db->insert('new_complaint',$data))
    ...
    $query = $this->db->query("select * from mis_complaint where token < '".$id."'order by token desc limit 1");
        ...
        {
            $query = $this->db->query("select a.mobile_no from user_other_details a where a.id='".$id."'");
