...
class Leave_dynamic_credit_sheet_model extends CI_Model {

    function __construct() {
        parent::__construct();
    }
	
	function check_prev_year_data($prev_year) {

        $myquery = "select * from  leave_balance_sheet_yearwise where year='$prev_year'";
	...
	function check_prev_acad_year_data($prev_acad_year) {

        $myquery = "select * from  leave_balance_sheet_yearwise where year='$prev_acad_year'";
	...
	function collect_prev_year_data() {

        $myquery = "select emp_no, el_bal, el_bal_actual,hpl_bal,cl_bal,rh_bal,pl_bal,dl_bal from leave_balance_sheet";
        $query = $this->db->query($myquery);
        if ($query->num_rows() > 0) {
	...
	function collect_prev_acad_year_data() {

        $myquery = "select emp_no, vl_bal from leave_balance_sheet where emp_no in (select emp_no from emp_basic_details where auth_id='ft')";
        $query = $this->db->query($myquery);
        if ($query->num_rows() > 0) {
	...
	function insert_prev_data($prev_data) {

        if ($this->db->insert('leave_balance_sheet_yearwise', $prev_data)) {            
           return true;
        } else {
            return false;
        }
    }
	function insert_prev_acad_data($prev_acad_data) {

        if ($this->db->insert('leave_balance_sheet_yearwise', $prev_acad_data)) {            
           return true;
        } else {
            return false;
        }
    ...
    function check_january_el_status($yr, $mon1) {

        $myquery = "select el_credit from  leave_dynamic_credit_sheet where year=? and month=? and el_credit <> ''";

        $query = $this->db->query($myquery, array($yr, $mon1));
    ...
    function check_july_el_status($yr, $mon2) {

        $myquery = "select el_credit from  leave_dynamic_credit_sheet where year=? and month=? and el_credit <> ''";

        $query = $this->db->query($myquery, array($yr, $mon2));
    ...
    function check_vl_status($yr, $mon2) {
    ...
    function check_january_hpl_status($yr, $mon1) {
    ...
    function check_july_hpl_status($yr, $mon2) {
    ...
    function check_cl_status($yr, $mon1) {
    ...
    function check_rh_status($yr, $mon1) {
    ...
    function check_pl_status($yr, $mon2) {
    ...
    function check_dl_status($yr, $mon2) {
    ...
    function insert_dynamic_leave_credit($data, $leave_type, $emp_type) {//To inserts data into leave_dynamic_credit_sheet and update data of leave_balance_sheet tables
        ...
        switch ($leave_type) {
            case 'EL':
				...
				if($result->el_bal>'315'){$result->el_bal='315';}
				$result->el_bal_actual=$result->el_bal_actual + $data['el_credit'];
                break;
...
//var_dump($data);die();
    ...
    function update_dynamic_leave_credit($data, $leave_type, $emp_type) {//To update data of leave_dynamic_credit_sheet and update data of leave_balance_sheet tables
        $this->db->where('emp_no', $data['emp_no']);
        $query = $this->db->get('leave_balance_sheet');
        $result = $query->result()[0];
    ...
    function update_el_for_faculty($data) {//It updates EL for faculty in leave_dynamic_credit_sheet and at the same time updates the data in leave_balance_sheet
        $this->db->where('emp_no', $data['emp_no']);
        $query = $this->db->get('leave_balance_sheet');
        $result = $query->result()[0];
    ...
    function insert_el_for_faculty($data) {//It inserts EL for faculty in leave_dynamic_credit_sheet and at the same time updates the data in leave_balance_sheet
        $this->db->where('emp_no', $data['emp_no']);
        $query = $this->db->get('leave_balance_sheet');
        $result = $query->result()[0];

        $result->el_bal = $result->el_bal + $data[el_credit];
		if($result->el_bal>'315'){
		$result->el_bal ='315';
		}
		$result->el_bal_actual=	$result->el_bal_actual + $data[el_credit];
		//var_dump($result->el_bal);die();
    ...
    function get_all_emp_nos($emp_type) {
        ...
        if ($emp_type == 'Faculty') {
			...
			$myquery ="select emp_no from leave_balance_sheet where emp_no in (select emp_no from emp_basic_details where auth_id='nfta' or auth_id='nftn' or auth_id='nft' or auth_id='pictp')";
        } else {
