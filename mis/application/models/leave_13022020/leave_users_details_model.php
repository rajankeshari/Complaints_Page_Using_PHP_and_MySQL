...
class Leave_users_details_model extends CI_Model {
    ...
    function get_designation_by_department_id ($dept_id)
    {
        ...
        $query = $this->db->query("SELECT DISTINCT designations.id, designations.name FROM designations INNER JOIN user_details INNER JOIN emp_basic_details ON designations.id = emp_basic_details.designation AND user_details.id = emp_basic_details.emp_no where dept_id LIKE '".$dept_id."' ORDER BY designations.name;");
        if($query->num_rows() > 0)
                return $query->result();
        else
                return FALSE;	 
    ...
    {
        $query = $this->db->query ("SELECT emp_basic_details.emp_no AS id from user_details INNER JOIN emp_basic_details ON user_details.id = emp_basic_details.emp_no where dept_id = '".$dept."' and designation = '".$designation."' ORDER BY salutation, first_name, middle_name, last_name;");
        if($query->num_rows() > 0)
                return $query->result();
        else
                return FALSE;	 
    ...
    function get_designation_by_dept_type($dept_type){

        $sql = "SELECT DISTINCT designations.id, designations.name  FROM ".
                " designations INNER JOIN user_details INNER JOIN emp_basic_details INNER JOIN departments".
                " ON designations.id = emp_basic_details.designation ".
                " AND user_details.id = emp_basic_details.emp_no ".
                " AND user_details.dept_id = departments.id ".
                " where departments.type LIKE '".$dept_type."' ORDER BY designations.name;";
...
//    }
    ...
    function is_any_of_this_auth_valid($emp_id , $auth_array){
        $result = $this->get_user_auth_type($emp_id);
        foreach ($result as $row){
...
//    }
    ...
    function getUserNameByID($emp_id)
    {
    ...
    //function added to load deptt &section as per rq. 16/9/15
