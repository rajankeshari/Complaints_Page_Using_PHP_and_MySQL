...
function get_stu_marks($sub_code,$session_year,$session,$course_id,$branch_id){
	...
	$sql="SELECT a.*, b.category_name,c.wtg as full_marks,b.marks
FROM cbcs_marks_upload a
INNER JOIN cbcs_marks_upload_description b ON a.id=b.marks_id
inner join cbcs_marks_dist_child c on b.category_id=c.pk
where a.subject_code='$sub_code' and a.admn_no='$stu_id'
...
function marksupmarks($sub_code,$session_year,$session,$course_id,$branch_id){
	$stu_id=$this->session->userdata('id');
	$sql="SELECT a.*, concat(a.dist_name,' ( ','Makeup Exam', ' )') as category_name,c.wtg AS full_marks,a.marks from
cbcs_markup_marks_upload a
INNER JOIN cbcs_marks_dist_child c ON a.dist_category_id=c.pk
WHERE a.sub_code='$sub_code' AND a.admn_no='$stu_id' AND a.session_year='$session_year' AND a.session='$session'
...
public function get_stu_subject($form_id,$stu_id,$sessionYear,$session,$section,$course_aggr_id,$course_id,$branch_id){
	...
	if(strpos($course_aggr_id, 'comm') !== false){
		...
		$condition="and (case when b.sub_type='online' then 1=1 else b.sub_group='$section' end) and (case when b.sub_type='online' then 1=1 else c.section=s.section end)";
		$branch_id="comm";
		$course_id="comm";
	}else{
	//	echo "not commm";
...
inner join cbcs_subject_offered_desc c on b.id=c.sub_offered_id and b.id=a.sub_offered_id
...
INNER JOIN old_subject_offered_desc c ON b.id=c.sub_offered_id and b.id=a.sub_offered_id
$sectionjoin
INNER JOIN user_details u ON c.emp_no=u.id
WHERE a.admn_no='$stu_id' AND a.form_id='$form_id' AND a.session_year='$sessionYear' AND a.session='$session' $condition AND c.coordinator='1' /* AND  b.branch_id='$branch_id' AND b.course_id='$course_id' */
ORDER BY SUBSTR(a.sub_category,3)+0 ASC)
...
function get_detail_weightage($subid,$syear,$sess,$empno,$course,$branch,$sem,$section,$sub_category){
	if($course=='comm' && $branch=='comm'){
		$extraclouse="and a.section='$section'";
	}else{
		$extraclouse="";
	}
...
where a.sub_code=? and a.session_year=? and a.`session`=? and a.emp_no=?
and a.course_id=? and a.branch_id=? and a.semester=? $extraclouse";
	$query = $this->db->query($sql,array($subid,$syear,$sess,$empno,$course,$branch,$sem));
	//echo" common ".  $this->db->last_query(); exit;
...
function get_detail_weightage_download($subid,$syear,$sess,$empno,$course,$branch,$sem,$section){
