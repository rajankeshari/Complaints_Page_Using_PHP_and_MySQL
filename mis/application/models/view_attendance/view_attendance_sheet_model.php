...
WHERE a.subject_offered_id=? AND a.group_no=? and b.admn_no=? ;";
	...
	function view_attendance_model_cbcs_new($data){
		
		if($data[section]!=''){
			$q="SELECT a.* FROM cbcs_class_engaged_course_wise a
LEFT JOIN cbcs_absent_table_course_wise b on b.class_engaged_id=a.id
WHERE a.session_year=? AND a.`session`=? 
and a.course_code=? AND a.engaged_by=? AND a.group_no=? AND a.section =? 
GROUP BY a.total_class order by a.total_class,a.date ";
	            $query = $this->db->query($q,array($data['session_year'],$data['session'],$data['sub_id'],$data['emp_no'],$data['group_no'],$data['section']));
	            return $query->result();
...
LEFT JOIN cbcs_absent_table_course_wise b on b.class_engaged_id=a.id
WHERE a.session_year=? AND a.`session`=? 
and a.course_code=? AND a.engaged_by=? AND a.group_no=? 
GROUP BY a.total_class order by a.total_class,a.date ";
	            $query = $this->db->query($q,array($data['session_year'],$data['session'],$data['sub_id'],$data['emp_no'],$data['group_no']));
	            return $query->result();

		}
}

		function view_attendance_model_cbcs_absent_new($data,$admn_no){
		if($data[section]!=''){
			$q="SELECT a.* FROM cbcs_class_engaged_course_wise a
LEFT JOIN cbcs_absent_table_course_wise b on b.class_engaged_id=a.id
WHERE a.session_year=? AND a.`session`=? 
and a.course_code=? AND a.engaged_by=? AND a.group_no=? AND a.section =? and b.admn_no=?
GROUP BY a.total_class order by a.date ";
	            $query = $this->db->query($q,array($data['session_year'],$data['session'],$data['sub_id'],$data['emp_no'],$data['group_no'],$data['section'],$data['admn_no']));
	            return $query->result();
...
LEFT JOIN cbcs_absent_table_course_wise b on b.class_engaged_id=a.id
WHERE a.session_year=? AND a.`session`=? 
and a.course_code=? AND a.engaged_by=? AND a.group_no=? and b.admn_no=?
GROUP BY a.total_class order by a.date ";
	            $query = $this->db->query($q,array($data['session_year'],$data['session'],$data['sub_id'],$data['emp_no'],$data['group_no'],$data['admn_no']));
	            return $query->result();
