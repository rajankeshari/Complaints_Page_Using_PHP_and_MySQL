...
WHERE hc_patient.visitor_id =" . $id);
 ...
 SELECT DISTINCT
  ...
  hc_patient.doc_id,concat(user_details.first_name,' ', user_details.middle_name,' ', user_details.last_name)as dname,hc_patient.visit_no,hc_patient.pres_no
FROM hc_patient
  INNER JOIN user_details
...
And date(hc_pat_remarks.obser_date) = '" . $visit . "'";
    ...
    if ($dt == '')
    {
   $q = $this->db->query("select distinct pid, prel,m_status , visit_date,visit_no,pres_no from hc_patient WHERE date(visit_date)= CURDATE() group BY 

pid,prel,visit_no order by visit_date desc"); 
    } 
        else {
                        
            /*$q = $this->db->query("select distinct pid, prel,m_status,visit_date,visit_no,pres_no from hc_patient WHERE date(visit_date)='" . $dt . "' group BY 

pid,prel,visit_no order by visit_date desc");*/
$q = $this->db->query("select distinct pid, prel,m_status,visit_date,visit_no,pres_no from hc_patient WHERE m_status='Pending' group BY pid,prel,visit_no order by visit_date desc");
...
$myquery = "update hc_patient_med_dues set m_status='Issued',hc_patient_med_dues.med_issued_by='".$this->session->userdata('id')."', hc_patient_med_dues.med_issued_on = date_format('".$pvdate."','%Y-%m-%d %H:%i:%s')
 WHERE hc_patient_med_dues.pid = '".$pid."' AND hc_patient_med_dues.prel = '".$prel."' and visit_no='".$pvno."' and pres_no='".$ppresno."' "  ;						
    ...
    function update_counter_master($pid, $prel, $pvdate,$pvno,$ppresno) {
