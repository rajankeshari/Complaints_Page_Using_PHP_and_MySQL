...
(SELECT a.*,CONCAT('o',a.sub_offered_id) AS sub_offered_id1 FROM old_stu_course a WHERE form_id=? AND admn_no=?)";
 ...
 function get_total_class_new1($syear,$sess,$scode,$group_no){
        $sql="SELECT COUNT(course_code) as cnt from cbcs_class_engaged_course_wise
WHERE session_year=?  AND SESSION=? AND course_code=? and group_no=?";

        //echo $sql;die();
        $query = $this->db->query($sql,array($syear,$sess,$scode,$group_no));
    ...
    function get_absent_count($session_year,$session,$subject_code,$admn_no)
    {
         $sql="SELECT COUNT(b.admn_no) as cnt from cbcs_class_engaged_course_wise a
INNER JOIN cbcs_absent_table_course_wise b ON a.id=b.class_engaged_id
WHERE a.session_year=? AND a.SESSION=? AND a.course_code=? AND b.admn_no=? ";

        //echo $sql;die();
        $query = $this->db->query($sql,array($session_year,$session,$subject_code,$admn_no));
    ...
    function get_student_from_group_distribution($session_year,$session,$subject_code,$admn_no){

$sql="(SELECT b.* FROM old_stu_course a 
INNER JOIN cbcs_optional_mapping b ON b.session_year=a.session_year AND b.`session`=a.`session` 
AND b.sub_code=a.subject_code
WHERE a.session_year=? AND a.`session`=?
AND a.subject_code=? AND a.admn_no=?
AND a.course=b.course_id AND a.branch=b.branch_id) union (SELECT b.* FROM cbcs_stu_course a 
INNER JOIN cbcs_optional_mapping b ON b.session_year=a.session_year AND b.`session`=a.`session` 
AND b.sub_code=a.subject_code
WHERE a.session_year=? AND a.`session`=?
AND a.subject_code=? AND a.admn_no=?
AND a.course=b.course_id AND a.branch=b.branch_id) ";

        //echo $sql;die();
        $query = $this->db->query($sql,array($session_year,$session,$subject_code,$admn_no,$session_year,$session,$subject_code,$admn_no));
    ...
    public function get_attendance_data($session_year,$session,$course_code,$group){
      
    $sql="SELECT a.* from cbcs_class_engaged_course_wise a
            WHERE a.session_year=? AND a.`session`=? AND a.course_code=? AND a.group_no=?";
        $query = $this->db->query($sql,array($session_year,$session,$course_code,$group));
   ...
   //echo $this->db->last_query();
    ...
    function get_course_code_name($session_year,$session,$course_code){

          $sql="(SELECT a.sub_code,a.sub_name FROM cbcs_subject_offered a
where a.session_year=? AND a.`session`=?
AND a.sub_code=?)
UNION
(SELECT a.sub_code,a.sub_name FROM old_subject_offered a
where a.session_year=? AND a.`session`=?
AND a.sub_code=?);
";
        $query = $this->db->query($sql,array($session_year,$session,$course_code,$session_year,$session,$course_code));
...
function get_stu_details($syear,$sess,$admn_no){

    $sql=" SELECT a.*,CONCAT_WS(' ',b.first_name,b.middle_name,b.last_name)AS sname FROM reg_regular_form a 
