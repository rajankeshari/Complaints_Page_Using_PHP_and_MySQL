...

{
	...
	function getAll_Suppliers()
	{
			...
			$query = $this->db->query("SELECT * FROM hc_supplier order by s_name");
			if($query->num_rows() > 0)
			{	
...
");
	...
	function getAll_Supp_byID($id)
	{
			
			$query = $this->db->select("*");
			$query = $this->db->from('hc_supplier');
			$query = $this->db->where('s_id',$id);
			$query=$this->db->get();
			if($query->num_rows() > 0)
			{	
	...
	function update_supp($id,$data){
     $this->db->where('s_id', $id);
     $this->db->update('hc_supplier', $data);  
    }
	public function check_supp_exist($id)
            {
             $this->db->where('s_name',$id);
             $query=$this->db->get('hc_supplier');
            if($query->num_rows()>0)
             {
              return true;
             }
