...
class Eso_mapping_model extends CI_Model {
    ...
    function get_eso_type($course_id,$sem){
    	$sql="SELECT a.* FROM cbcs_coursestructure_policy a
WHERE a.course_id='$course_id' AND a.sem='$sem' AND a.course_component LIKE 'ESO%'";
...
";
    ...
    function get_eso_list($sy,$sess,$eso){
    	$sql="SELECT a.*
FROM cbcs_subject_offered a
WHERE a.session_year='$sy' AND a.`session`='$sess' AND a.sub_category='$eso'";
$query=$this->db->query($sql);
	//echo $this->db->last_query();
    return $query->result();
    }
