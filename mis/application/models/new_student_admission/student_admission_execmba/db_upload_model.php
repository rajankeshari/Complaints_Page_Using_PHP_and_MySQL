...
{
	var $table = 'stu_temp_db_exemba'; // table name in the database in which data of newly admitted students are stored.
	...
	function get_admission_no($q)
	{
		...
		$this->db->limit(10);
		$query = $this->db->get('stu_temp_adm_no_exemba');
		if($query->num_rows() > 0)
		{
	...
	function check($Adm1)
	{
	    $query1 = $this->db->get_where('stu_temp_adm_no_exemba', array('admission_no' => $Adm1));
	   $row1=$query1->row();
	...
	{   
		...
		$this->db->update('stu_temp_adm_no_exemba',$arr);
	}
	
	function getuser($reg_id2)
	{
	   $query2 = $this->db->get_where('stu_temp_db_exemba', array('reg_id' => $reg_id2));
	...
	function give_admission_no($first_pat,$first_adm,$last_adm)
	{
		...
		$table_name='stu_temp_adm_no_exemba';
