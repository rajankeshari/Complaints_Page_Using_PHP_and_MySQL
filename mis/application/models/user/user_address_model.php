...
class User_address_model extends CI_Model
{
	...
	function insertPendingCorrespondenceAddr($data)
	{
	...
	function updatePendingCorrespondenceAddrById($data,$id)
	{
		$this->db->update('pending_'.$this->table,$data,array('id'=>$id,'type'=>'correspondence'));
	}
	...
	function deletePendingCorrespondenceAddrById($id)
	{
		$this->db->delete('pending_'.$this->table,array('id'=>$id,'type'=>'correspondence'));
	}
