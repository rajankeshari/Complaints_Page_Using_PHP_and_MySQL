...
class Payment_admin_query_model extends CI_Model{
  ...
  public function get_list_student_paid_online()
  {
    ...
    WHERE a.payment_mode='online' and a.payment_status='1' AND a.session_year='2020-2021' AND a.`session`='Monsoon' group by a.admn_no , e.admn_no , d.admn_no , f.form_id";
    $query = $this->db->query($sql);
    return $query->result_array();
  ...
  public function get_list_student_paid_online_failure()
  {
    ...
    INNER JOIN reg_regular_fee d ON d.form_id = e.form_id
    INNER JOIN reg_regular_form f ON f.form_id = d.form_id
    WHERE a.payment_mode='online' and a.payment_status='2' AND a.session_year='2020-2021' AND a.`session`='Monsoon' group by a.admn_no , e.admn_no , d.admn_no , f.form_id";
    $query = $this->db->query($sql);
    return $query->result_array();

  }

  public function get_list_student_paid_offline()
  {

    $sql = "SELECT * FROM bank_fee_details a 
    INNER JOIN reg_regular_fee d ON d.admn_no = a.admn_no
    INNER JOIN reg_regular_form f ON f.form_id = d.form_id
    WHERE a.payment_mode='offline' AND f.session_year='2020-2021' AND f.`session`='Monsoon' AND f.hod_status = '1' AND f.acad_status = '1' group by a.admn_no , d.admn_no , f.form_id";
    $query = $this->db->query($sql);
    return $query->result_array();
 
  }

  public function get_list_student_not_paid()
  {

    $sql = "SELECT * FROM bank_fee_details a 
    INNER JOIN reg_regular_fee d ON d.admn_no = a.admn_no
    INNER JOIN reg_regular_form f ON f.form_id = d.form_id
    WHERE a.payment_status=0 AND f.session_year='2020-2021' AND f.`session`='Monsoon' AND f.hod_status = '1' AND f.acad_status = '1' group by a.admn_no , d.form_id , f.form_id";
    $query = $this->db->query($sql);
...
// WHERE a.id=?";
  ...
  public function get_not_paid_admission_no_list() {
