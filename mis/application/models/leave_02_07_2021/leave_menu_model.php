...
class leave_menu_model extends CI_Model {
    ...
    function getMenu() {
       ...
       // $menu['emp']['Leave Management']['Station Leave']['Station Leave Archive'] = site_url('leave/leave_station/stationLeaveHistory');
		...
		$menu['emp']['Leave Management']['My Returned Applications'] = site_url('leave/leave_all_kind_application/my_returned_applications');
        $menu['emp']['Leave Management']['My Current Leave Balance'] = site_url('leave/leave_balance_individual');

		/*$ADMIN_ARRAY for Director only*/		
        $menu['dt'] = array();
        $menu['dt']['Leave Management']['Leave Applications'] = site_url('leave/leave_all_kind_application/approving_authority/'.dt);
        $menu['dt']['Leave Management']['My History'] = site_url('leave/leave_all_kind_application/history_of_authority/'.dt);
...
//echo $row;echo '<br>';
			if($row=='hod' || $row=='hos'){
				...
				$this->load->model('leave/leave_all_kind_application_model','',TRUE);
				$row1=$this->leave_all_kind_application_model->get_new_auth_id($emp_id);
            ...
            $menu[$row]['Leave Management']['Leave Applications'] = site_url('leave/leave_all_kind_application/approving_authority/'.$row1);
            $menu[$row]['Leave Management']['My History'] = site_url('leave/leave_all_kind_application/history_of_authority/'.$row1);
        ...
        foreach (Leave_constants::$OTHERS_UNIQUE_ADMIN as $row) {
			...
			$menu[$row]['Leave Management']['Application Form'] = site_url('leave/leave_all_kind_application/admin_side_application/'.$row);
			$menu[$row]['Leave Management']['Approved Leave Applications'] = site_url('leave/leave_all_kind_application/admin_side_archive/'.$row);
			$menu[$row]['Leave Management']['Approved Leave Applications History'] = site_url('leave/leave_all_kind_application/admin_side_archive_history/'.$row);
