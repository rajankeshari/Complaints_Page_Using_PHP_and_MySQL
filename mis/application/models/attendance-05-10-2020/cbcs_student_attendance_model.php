...
class cbcs_student_attendance_model extends CI_Model {
...
where a.admn_no='" . $id . "';");
    ...
    function get_current_status($id) {
        ...
        $query = "select a.session_year,a.`session`,a.semester from reg_regular_form a
...
and a.hod_status='1' and a.acad_status='1')";
    ...
    function get_subject_mapping_jrf($syear,$sess,$aggr_id,$sub_id){

         $sql="select a.*,b.sub_id from subject_mapping a
inner join subject_mapping_des b on b.map_id=a.map_id
where a.session_year=? and a.`session`=?
and a.aggr_id=? and b.sub_id=?";

        //echo $sql;die();
        $query = $this->db->query($sql,array($syear,$sess,$aggr_id,$sub_id));
    ...
    function get_total_class_jrf($mapid,$subid){
        $sql="select a.* from total_class_table_jrf a where a.map_id=? and a.sub_id=?";

        //echo $sql;die();
        $query = $this->db->query($sql,array($mapid,$subid));
