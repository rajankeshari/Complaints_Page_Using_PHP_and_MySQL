...
class Cbcs_attendance_new_model extends CI_Model {
    ...
    function get_student_list($subcode,$section,$syear,$sess,$course_id){
        ...
        WHERE a.subject_code = ? and d.status='A' AND a.sub_category_cbcs_offered=?
        AND a.session_year=? AND a.`session`=?  and (a.remark2='1' or a.remark2='3') group by c.id ORDER BY b.course_id,b.branch_id,c.id" ;
        
         $query = $this->db->query($sql,array($subcode,$section,$syear,$sess));
        ...
        INNER JOIN user_details c ON c.id=b.admn_no
        inner join users d on d.id=b.admn_no
        WHERE a.subject_code = ? and d.status='A'
        AND a.session_year=? AND a.`session`=?  and (a.remark2='1' or a.remark2='3') group by c.id ORDER BY b.course_id,b.branch_id,c.id" ;
        
         $query = $this->db->query($sql,array($subcode,$syear,$sess));
	...
	function count_student($subject_code,$section,$syear,$sess,$course_id){
        ...
        $query = $this->db->query($sql,array($subject_code,$section,$syear,$sess));
        } 
        ...
        $query = $this->db->query($sql,array($subject_code,$syear,$sess));
