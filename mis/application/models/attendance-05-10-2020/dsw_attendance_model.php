	...
	public function getStudents($data)
	{
		...
		foreach ($result as $row) 
		{
			$query = $this->db->query("SELECT course_id , branch_id , semester as semster  
									   ...
									   WHERE admn_no = '$row->admission_id' and hod_status='1' and acad_status='1' order by form_id desc ");
			($query = $query->result()) ;
		...
		foreach ($result as $row) 
		{
			...
			//print_r($row);
			$query=$this->db->query("SELECT * FROM absent_table_defaulter
									 WHERE map_id = $row->map_id AND sub_id = '$row->subject_id'
									 AND admn_no = '$row->admission_id' 
									 AND (Remark='none' || Remark='late_reg')
									 AND status != 0  ");
			//echo "bhagat";
			($temp_1 = $query->result()) ;
			...
			$percent = (($temp_1[0]->total_class - $temp_1[0]->tot_absent)) *100;
			$percent = (float)(($percent/$temp_1[0]->total_class));

			$percent = round($percent,2);
			 $row->percent = $percent;
			  	    $row->total_class = $temp_1[0]->total_class ;
					$row->tot_absent  = $temp_1[0]->tot_absent ;
					$row->percentage  = $temp_1[0]->percentage ;
		}
...
public function send_exam_section($result)
	{
		...
		{	
				$this->load->database();
				$status=2;
				$this->db->query("UPDATE absent_table_defaulter 
								 SET status=$status 
	...
	function getModFromtech($subid,$stuid,$map_id=null){
		
		    if($map_id){
            $mid=$this->get_subject_mapping($map_id);
            $sid=$this->get_session_id($mid->session_year,$mid->session,$mid->branch_id,$mid->course_id,$mid->semester);
            //$q=$this->db->get_where('Attendance_remark_table',['sub_id'=>$subid,'admn_no'=>$stuid,'session_id'=>$sid->session_id]);
            $sql="SELECT sum(count)as count,group_concat(Remark) as Remark FROM Attendance_remark_table WHERE 
admn_no='".$stuid."' AND sub_id='".$subid."' AND session_id='".$sid->session_id."' group by admn_no,sub_id,session_id";
		$q=$this->db->query($sql);
            
            }else{
               //$q=$this->db->get_where('Attendance_remark_table',['sub_id'=>$subid,'admn_no'=>$stuid]);  
                $sql="SELECT sum(count)as count,group_concat(Remark) as Remark FROM Attendance_remark_table WHERE 
admn_no='".$stuid."' AND sub_id='".$subid."'  group by admn_no,sub_id";
		$q=$this->db->query($sql);
            }
        ...
        function get_subject_mapping($mapid){
            $q=$this->db->get_where('subject_mapping',['map_id'=>$mapid]);
			if($q->num_rows() >0)
			return	$q->row();
		return false;
            
        }
        function get_session_id($sy,$sess,$bid,$cid,$sem){
            $q=$this->db->get_where('session_track',['session_year'=>$sy,'session'=>$sess,'branch_id'=>$bid,'course_id'=>$cid,'semester'=>$sem]);
			if($q->num_rows() >0)
			return	$q->row();
		return false;
		...
		public  function  get_tot_class_by_sub_id_map_id($sub_id,$map_id) {
