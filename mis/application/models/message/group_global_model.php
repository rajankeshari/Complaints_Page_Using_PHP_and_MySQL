...
class Group_global_model extends CI_Model
{
	...
	var $table_groups='message_group_global';
	...
	{		
		//for inserting member in group not individual
			$query2=$this->db->query("SELECT * from message_group_global where group_id='$data1' and user_id='$data3'");
		
			if($query2->num_rows()==0)
			{
				$query = "INSERT INTO message_group_global VALUES ('$data1','$data3','$data4')";
				$this->db->query($query);
			}
	...
	{
		$query="DELETE FROM message_group_global WHERE group_id='$group_name'";
		$this->db->query($query);
	...
	{		
		...
		$query = "DELETE FROM message_group_global 
