...
class Post_message_model extends CI_Model
{
	...
	function get_mobile_no($data=array())
	{
		...
		if($data['type'] === "ind" || $data['type'] === "grp"){
			$users = $data['users'];
			$mobileAssoc = $this->db->select('mobile_no')->where_in('id',$users)->get('user_other_details')->result_array();
			...
			if($data['subtype'] === "stu"){
				...
				$mobileAssoc =  $this->db->select('O.mobile_no')->from('user_details as U')
												->where($cond)
												->join('stu_academic as S','S.admn_no = U.id','inner')
												->join('user_other_details as O','O.id = U.id','inner')
												->get()->result_array();
			}
			...
			$mobileAssoc = $this->db->select('O.mobile_no')
									->from('emp_basic_details as E')
									->join('user_other_details as O','O.id = E.emp_no','inner')
									->get()->result_array();
		}
		...
		else if($data['type']==='stu')
		{
			...
			$mobileAssoc = $this->db->select('mobile_no')->get('user_other_details')->result_array();
		}
