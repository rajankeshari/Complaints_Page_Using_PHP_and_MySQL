...
class Guard_model extends CI_Model
{
	...
	{
		$guard = $this->get_details_of_a_guard_toadd($data['Regno']);
                $guard1 = $this->get_details_of_a_guard_toadd1($data['Regno']);
          
		if(count($guard) == 0 && count($guard1) == 0 )
		{
	...
	function get_details_of_a_guard($regno)
	{
                ...
                $this->db->where('rid',$regno);
		$query = $this->db->get('guard_guards');
		
		return $query->row_array();
	}
        function get_details_of_a_guard_toadd($regno)
	{
        ...
        function get_details_of_a_guard_toadd1($regno)
	{
		
                $this->db->where('Regno',$regno);
		$query = $this->db->get('guard_guards_archive');
		echo $this->db->last_query();
		return $query->row_array();
	}
