    ...
    function get_depart_details($sy, $sess, $did, $etype, $dec_type = 'F', $sec = null) {
        ...
        if ($etype == 'regular' || $etype == 'jrf' || $etype == 'jrf_spl' || $etype == 'prep') {
            ...
            if ($etype == 'regular') {
                ...
                $str_crs_replace = ($etype == 'prep' ? " and a.course_id='prep'" : " and a.course_id='jrf'" );
                $semester_replace1 = "";
                $semester_replace2 = "";
                $semester_replace3 = "";
            }
...
";
  ...
  select 
  (CASE WHEN (f.status = 'FAIL') THEN 'INC' ELSE  ( CASE WHEN ((f.hstatus='Y') ) THEN f.core_cgpa ELSE  f.cgpa end) END) AS cgpa,
...
/*GROUP_CONCAT((CONCAT(b.sub_code,'(',b.grade,')')) SEPARATOR ',') AS sublist, */ 
 /*GROUP_CONCAT((CONCAT( REPLACE(c.name, ',', ''),'#',b.sub_code,'#',COALESCE(b.theory,''),'#',COALESCE(b.sessional,''),'#',COALESCE(b.practical,''),'#',COALESCE(b.total,''),'#',b.cr_pts,'#',b.cr_hr,'#',CAST(CONCAT(c.lecture,'-',c.tutorial,'-',c.practical) AS CHAR CHARACTER SET utf8),'#',b.grade)) SEPARATOR ',') AS sublist,*/
SUM(IF (  (b.grade = 'F'            
     and
        (case  
...
(CASE WHEN (a.status = 'FAIL') THEN 'INC' ELSE 
 ...
 ( CASE WHEN ((a.hstatus='Y') ) THEN a.core_status ELSE  a.status end) AS core_status,
  ...
  (CASE WHEN (a.hstatus='Y') then (CASE WHEN (a.status = 'FAIL') THEN 'INC' ELSE a.cgpa END) else 'N/A' end) AS H_cgpa,a.session_yr,a.session,a.semester,a.hstatus
...
LEFT JOIN subjects AS c ON b.sub_code=c.id
