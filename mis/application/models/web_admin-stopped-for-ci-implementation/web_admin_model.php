				...
				public function getusersdetails($userid,$utype){
					if(!empty($userid) && !empty($utype)){
						if($utype=='stu'){
							...
							$query=$this->db->query("SELECT ud.*,ebd.*,email.domain_name,ua.contact_no FROM user_details ud JOIN emp_basic_details ebd on ud.id=ebd.emp_no JOIN emaildata_emp email on ud.id=email.emp_id JOIN user_address ua ON ud.id=ua.id where ud.id='$userid'  group by ud.id");
