...
class Student_validate extends MY_Controller
{
	...
	{
		parent::__construct(array('acad_ar','acad_dr'));
	}
	...
	function index()
	{
		...
		//var_dump($data["users_to_validate"]);
	...
	function validation($admn_no='')
	{
		...
		$this->load->model('student/student_details_to_approve','',TRUE);
		if(!$this->student_details_to_approve->get_all_stu_details_by_id($admn_no))
		{
	...
	function delete_pending_data($stu_id)
	{
		...
		if($this->user_details_model->getUserById($stu_id))
		{
			...
			$this->student_fee_details_model->update_by_id($stu_fee_details,$stu_id);
			$this->student_academic_model->update_by_id($stu_academic,$stu_id);           
                        //print_r($stu_education_details);
                        $this->student_education_details_model->update_by_sl_id( $stu_education_details,$stu_id); //Added as education details modification required Author:RituRaj dated:16-sept15
		}
		else
		{
		...
		$this->student_details_to_approve->deleteDetailsWhere(array('admn_no'=>$admn_no));
		...
		$this->student_fee_details_model->deletePendingDetailsWhere(array('admn_no'=>$stu_id));
		$this->student_details_model->deletePendingDetailsWhere(array('admn_no'=>$stu_id));
		$this->student_education_details_model->deletePendingDetailsWhere(array('admn_no'=>$stu_id));
