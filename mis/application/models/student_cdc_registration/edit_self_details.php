...
class Edit_self_details extends MY_Controller
{
	...
	function index($error = '')
	{
		...
		//$this->load->view('student/edit/edit_self_details',$data);
		//$this->drawFooter();
	...
	//		}
		...
		if(strtolower($user_details->category) != strtolower($this->input->post('category1'))) //categoy1 is from hidden field due to disbaled dropdown of category saving  0 in category
		{
		...
		if($stu_basic_details->parent_email_id != $this->input->post('parent_email_id'))
		{
			$details_modified_array = array_values($details_modified_array);
			$details_modified_array[] = 'Parent\'s Email Id';
		}
		...
		if($_FILES['photo']['name'])
		{
			...
			if($upload == FALSE && $_FILES['photo']['name']!="")
			{
				//var_dump('no image');
				$this->session->set_flashdata('flashError','Student '.$stu_id.' image upload failed. Please upload file less than 200KB.');
				//redirect("home");
			...
			else
			{
				...
				$image_path = 'student/'.strtolower($stu_id).'/'.$upload['file_name'];
				//$image_path = strtolower($image_path);
				$details_modified_array = array_values($details_modified_array);
      ...
      $this->session->set_userdata('photopath', $image_path);
        //$image=array('photopath' =>$image_path);
		//$this->user_details_model->updateById($image,$stu_id);
        
        $pic=strrchr( $image_path, '/' ); 
        $pic=strchr($pic,'s');

		$dir = 'assets/images/student/'.strtolower($stu_id);
$leave_files = array($pic);
...
//--------------------------------------------------------------------------------------------------------------------------------                        
			...
			$user_details = array(
				//'id' => $stu_id ,
				'salutation' => $user_details->salutation, //$this->input->post('salutation') ,
				'first_name' => $user_details->first_name,//ucwords(strtolower($this->authorization->strclean($this->input->post('firstname')))) ,
				'middle_name' => $user_details->middle_name,//ucwords(strtolower($this->authorization->strclean($this->input->post('middlename')))) ,
				'last_name' => $user_details->last_name,//ucwords(strtolower($this->authorization->strclean($this->input->post('lastname')))) ,
				'sex' => $this->input->post('sex') ,
				'category' => $user_details->category,//$this->input->post('category1') ,
			...
			$stu_details = array(
				...
				'stu_type' => $stu_basic_details->stu_type, //$this->input->post('stu_type') ,
			...
			$stu_academic = array(
				//'admn_no' => $stu_id ,
				'auth_id' => $stu_basic_details->stu_type ,
				...
				'course_id' => $stu_academic_details->course_id ,     //change
				'branch_id' => $stu_academic_details->branch_id ,     //change
				'semester' => $stu_academic_details->semester ,     //change
...
*/			}
			...
			//$res = $this->user_auth_types_model->getUserIdByAuthId('acad_ar');
                     //        print_r($stu_education_detail); die();
			$this->db->trans_start();
			...
			$this->user_other_details_model->updateById($user_other_details,$stu_id);
                      // print_r($correspondence_address); die();
			$this->user_address_model->updatePresentAddrById($user_address[0],$stu_id);
			$this->user_address_model->updatePermanentAddrById($user_address[1],$stu_id);
			if($correspondence_address)
				if($user_address[2])
					$this->user_address_model->updateCorrespondenceAddrById($user_address[2],$stu_id);
				else
					$this->user_address_model->deleteCorrespondenceAddrById($stu_id);
			else
				if($user_address[2])
					$this->user_address_model->insert($user_address[2]);
            ...
            $this->student_education_details_model->deleteEducationDetailsWhere(array('admn_no'=>$stu_id));
            $this->student_education_details_model->insert_batch($stu_education_detail);
		...
		/*	$this->user_details_model->insertPendingDetails($user_details);
            ...
            if($this->db->trans_status()=== FALSE){
                     $this->session->set_flashdata('flashDanger','Student '.$stu_id.' data not modified.'.$this->db->_error_message());   
                        }
                        else{
				$this->session->set_flashdata('flashSuccess','Student '.$stu_id.' data succssfully modified.');
            }
            if($upload==false) {
				$this->session->set_flashdata('flashDanger','Student '.$stu_id.' profile picture not successfully updated. Please upload file less than 200KB.');
            }         
            else{
				$this->session->set_flashdata('flashSuccess','Student '.$stu_id.' data succssfully modified.');
            }
	...
	if(isset($_FILES[$name]['name']))
    	{
            ...
            else
			{
                ...
                //$filename='stu_'.strtolower($stu_id).$ext;
            }
        }
        else
        {
