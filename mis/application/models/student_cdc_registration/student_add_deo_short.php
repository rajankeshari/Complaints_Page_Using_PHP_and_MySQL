...
class Student_add_deo_short extends MY_Controller
{
	function __construct()
	{
		parent::__construct(array('acad_da1','exam_dr','acad_ar','acad_dr'));
	}
...
//			$this->form_validation->set_rules('contact3','Address Contact 3','required|regex_match[/^[0-9]{10}$/]');
	...
	//	$this->form_validation->set_rules('mobile','Mobile No','required|regex_match[/^[0-9]{10}$/]');
		...
		//$this->form_validation->set_rules('fee_paid_amount','Fee Paid Amount','numeric');
                if(!$this->input->post('stu_type') == 'jrf'){
		if($this->input->post('branch') == 'none' || $this->input->post('course') == 'none' || $this->form_validation->run() === FALSE)
		{
			...
			redirect('student/student_add_deo_short');
		}
                }
