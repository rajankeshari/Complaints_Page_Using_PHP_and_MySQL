...
class Student_add extends CI_Controller//MY_Controller
{
	...
	public function insert_basic_details($stu_id)
	{
		...
		if($admn_based_on === 'others'){
			$this->form_validation->set_rules('mode_of_admission','Other Mode of Admission','trim|required');
			$this->form_validation->set_rules('other_rank','Other Rank','trim|required');
		}
		/*else if($admn_based_on === 'iitjee')
		{
			$this->form_validation->set_rules('iitjee_rank','IIT JEE Rank','trim');
			$this->form_validation->set_rules('iitjee_cat_rank','IIT JEE Category Rank','trim');
		}*/
		...
		if($this->input->post('branch') == 'none' || $this->input->post('course') == 'none' || $this->form_validation->run() === FALSE)
		{
			...
			echo '<html>Either your Javascript is Off or due to some other problem the javascript did not work and some incorrect details were reveived by the Server.<br> If your javascript is not turned off then press the back button of browser and make sure the following fields are filled properly : <br>1. All the Contact numbers (except the Parent/Guardian Landline number) should be of 10 digits only. <br>2. The combination of the Department, Course and Branch is correct.<br><br> If still facing problem contact Mr. Rajesh K. Mishra, CSE Dept.<html>';
			return;
		...
		$this->load->model('course_structure/basic_model','',TRUE);
 if($this->input->post('stu_type')!="jrf"  &&  $this->input->post('stu_type')!="pd"){
		...
		//	}
			...
			while($i<$n)
			{
				...
				$stu_education_details[$i]['sno'] = $i+1;
				$stu_education_details[$i]['exam'] = strtolower($this->authorization->strclean($exam[$i]));
				$stu_education_details[$i]['specialization'] = strtolower($this->authorization->strclean($branch[$i-2]));
				$stu_education_details[$i]['institute'] = strtolower($this->authorization->strclean($clgname[$i]));
				$stu_education_details[$i]['year'] = $year[$i];
				$stu_education_details[$i]['grade'] = strtolower($this->authorization->strclean($grade[$i]));
				$stu_education_details[$i]['division'] = strtolower($div[$i]);
