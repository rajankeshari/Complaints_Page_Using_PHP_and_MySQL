...
class Student_add_deo extends MY_Controller
{
	...
	{
		parent::__construct(array('acad_da1','acad_ar','acad_dr'));
	}
...
*/
	...
	//	{
		 	$this->form_validation->set_rules('guardian_name','Guardian Name','trim');
		 	$this->form_validation->set_rules('guardian_relation_name','Guardian Relation','trim');
	//	}
	//	else
	//	{
...
//print_r($admn_based_on); 
//print_r($mode_of_admission); die();			
		}
		...
		// I have commented the complete if part and addressed the issue of 2016dr1089 as the error was only because of this if condition , move it back to original
		if($this->input->post('branch') == 'none' || $this->input->post('course') == 'none' || $this->form_validation->run() === FALSE)
		{
			$this->session->set_flashdata('flashError',mode_of_admission);
