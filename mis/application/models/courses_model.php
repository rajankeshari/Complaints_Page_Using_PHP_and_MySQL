...
class Courses_model extends CI_Model
{
	...
	{
		//print_r($dept_id);
		$command = "SELECT * FROM courses INNER JOIN 
		(SELECT DISTINCT course_branch.course_id FROM course_branch INNER JOIN
		 (SELECT course_id FROM dept_course
		  WHERE dept_id='$dept_id') as temp_table_1
		   ON course_branch.course_id = temp_table_1.course_id ) as temp_table_2 
			ON temp_table_2.course_id = courses.id ";
