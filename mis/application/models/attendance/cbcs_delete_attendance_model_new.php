...
class Cbcs_delete_attendance_model_new extends CI_Model {
...
order by date1 desc" ;
	...
	function backup_from_class_engage($syear,$sess,$groupno,$subcode,$section,$engaged_by,$dtype){
		...
		if($course_id=='comm'){
			$myquery = "INSERT INTO cbcs_class_engaged_course_wise_backup SELECT * FROM cbcs_class_engaged_course_wise WHERE session_year=?  and session=? and group_no=? and course_code=? AND section=? AND engaged_by=?   ".$where ;
             $query = $this->db->query($myquery,array($syear,$sess,$groupno,$subcode,$section,$engaged_by));
			
		}else{
			$myquery = "INSERT INTO cbcs_class_engaged_course_wise_backup SELECT * FROM cbcs_class_engaged_course_wise WHERE session_year=?  and session=? and group_no=? and course_code=?  AND engaged_by=? ".$where ;
             
		$query = $this->db->query($myquery,array($syear,$sess,$groupno,$subcode,$engaged_by));
	...
	function backup_from_absnet_table($syear,$sess,$groupno,$subcode,$section,$engaged_by,$dtype){
		...
		if($course_id=='comm'){
			$myquery = "INSERT INTO cbcs_absent_table_course_wise_backup SELECT * FROM cbcs_absent_table_course_wise WHERE class_engaged_id IN (SELECT id FROM cbcs_class_engaged_course_wise WHERE session_year=?  and session=? and group_no=? and course_code=? AND section=? AND engaged_by=? ".$where."  ) " ;
		$query = $this->db->query($myquery,array($syear,$sess,$groupno,$subcode,$section,$engaged_by,$dtype));
			
		}else{
       
		$myquery = "INSERT INTO cbcs_absent_table_course_wise_backup SELECT * FROM cbcs_absent_table_course_wise WHERE class_engaged_id IN (SELECT id FROM cbcs_class_engaged_course_wise WHERE session_year=?  and session=? and group_no=? and course_code=?  AND engaged_by=? ".$where."  ) " ;
		$query = $this->db->query($myquery,array($syear,$sess,$groupno,$subcode,$engaged_by,$dtype));
		}
        //echo $this->db->last_query();die();
        if($this->db->affected_rows()){
          return true;
	...
	function delete_from_class_engage_table($syear,$sess,$groupno,$subcode,$section,$engaged_by,$dtype){
		...
		if($course_id=='comm'){
			$myquery = "DELETE FROM cbcs_class_engaged_course_wise WHERE session_year=? and session=? and group_no=? and course_code=? and section=?  AND engaged_by=? ".$where ;
      
        
		$query = $this->db->query($myquery,array($syear,$sess,$groupno,$subcode,$section,$engaged_by,$dtype));
			
		}else{
	$myquery = "DELETE FROM cbcs_class_engaged_course_wise WHERE session_year=? and session=? and group_no=? and course_code=?   AND engaged_by=?  ".$where ;
	...
	function delete_from_absnet_table($syear,$sess,$groupno,$subcode,$section,$engaged_by,$dtype){
	...
	if($course_id=='comm'){
		$myquery = "DELETE FROM cbcs_absent_table_course_wise WHERE class_engaged_id IN (SELECT id FROM cbcs_class_engaged_course_wise WHERE session_year=? and session=? and course_code=? AND engaged_by=?  AND group_no=? and section=? ".$where.") " ;
		$query = $this->db->query($myquery,array($syear,$sess,$subcode,$engaged_by,$groupno,$section));

	}else{
		$myquery = "DELETE FROM cbcs_absent_table_course_wise WHERE class_engaged_id IN (SELECT id FROM cbcs_class_engaged_course_wise WHERE session_year=? and session=? and course_code=? AND engaged_by=?  AND group_no=?  ".$where.") " ;
		$query = $this->db->query($myquery,array($syear,$sess,$subcode,$engaged_by,$groupno));
	 ...
	 function insert_into_cbcs_absent_table_bkp_auth($data){
        if($this->db->insert('cbcs_absent_table_bkp_auth_course_wise',$data))
			return  true;
		else
			return FALSE;
    }
