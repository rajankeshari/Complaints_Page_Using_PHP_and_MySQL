...
inner join user_details x on a.admn_no=x.id
where a.subject_code='$sub_code' and a.sub_offered_id='$sub_offerd_id' and a.session_year='$session_year' and a.`session`='$session' order by a.admn_no)) x 
inner join reg_regular_form rrf on x.admn_no=rrf.admn_no and rrf.hod_status='1' and rrf.acad_status='1' and x.session=rrf.`session` and x.session_year=rrf.session_year
order by x.admn_no
  ";
}
