...
class Project_inst_pdf_details_model extends CI_Model
{
	var $table = 'project_inst_pdf_details';
	...
	function getVerifiedEmployeesId(){
...
//			else if($query->num_rows() == 1 )	return $query->row();
	...
	{
		$query = "SELECT a.emp_no, a.salutation, a.first_name, a.middle_name, a.last_name, b.status, b.reason FROM pending_project_inst_pdf_details AS a JOIN project_emp_validation_details AS b ON a.emp_no = b.emp_no";
		//echo $query ; die();
		$query = $this->db->query($query);
		//echo $this->db->last_query(); die();
	...
	function getPendingDetailsById($emp_id = '')
	{
        ...
        $query = $this->db->query("select * from pending_project_inst_pdf_details where emp_no = '$emp_id'");
	...
	function getPendingDetailsRejectedById($emp_no = '')
	{
		...
		else
		{
			...
			//$query= "select * from pending_project_inst_pdf_details"
		//	$query = "SELECT a.emp_no, a.nature_of_hiring,a.salutation, a.first_name, a.middle_name, a.last_name,a.sex b.status, b.reason FROM pending_project_inst_pdf_details AS a JOIN project_emp_validation_details AS b ON a.emp_no = b.emp_no";
		$query= "select a.*, b.status, b.reason from pending_project_inst_pdf_details AS a JOIN project_emp_validation_details AS b ON a.emp_no = b.emp_no where a.emp_no='$emp_no'";
		//$query = "SELECT a.emp_no, b.status, b.reason FROM pending_project_inst_pdf_details AS a JOIN project_emp_validation_details AS b ON a.emp_no = b.emp_no where a.emp_no =$emp_no  and b.status=`"rejected"` ";
		$query = $this->db->query($query);
			//echo $query->num_rows() ;
		//	echo "here";die();
