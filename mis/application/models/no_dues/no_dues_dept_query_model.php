...
//	*/
	...
	public function get_students_admn_details($admn_no)
	{
		...
		$sql = "SELECT user_details.id,user_details.photopath, first_name, middle_name, last_name, user_details.dept_id, reg_regular_form.semester, reg_regular_form.course_id
		FROM user_details
		INNER JOIN stu_academic ON user_details.id = stu_academic.admn_no
		INNER JOIN reg_regular_form ON reg_regular_form.admn_no = stu_academic.admn_no
		WHERE stu_academic.admn_no LIKE '$admn_no'
		ORDER BY reg_regular_form.form_id desc LIMIT 1";
	...
	public function student_details_table($session_year,$session,$dept_id, $course,$branch, $sem){
		/*
		...
		*/
		...
		// 	"AND stu_academic.course_id LIKE '$course'";
            ...
            FROM dept_course a
            JOIN course_branch b ON a.course_branch_id=b.course_branch_id AND a.dept_id='".$dept_id."'
            ...
            WHERE hod_status='1' AND acad_status='1' and session_year = '".$session_year."' and `session` = '".$session."' and semester = '".$sem."' AND UPPER(course_id)='".$course."' AND branch_id='".$branch."')B ON A.course_id=B.course_id AND A.branch_id=B.branch_id
...
//			$res[$i]['dept_name'] = $this->get_dept_name($res[$i]['branch_id']);
