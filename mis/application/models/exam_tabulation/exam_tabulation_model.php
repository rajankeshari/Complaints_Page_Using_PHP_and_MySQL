...
class Exam_tabulation_model extends CI_Model {
    ...
    function getStu_basic_dat($admn_no,$session_year,$session,$semester,$dept,$course,$branch){
       ...
       if($session_year == ''){
         $sql="SELECT a.form_id,a.admn_no, CONCAT_WS(' ',d.first_name,d.middle_name,d.last_name) AS stu_name,f.semester,a.course_id,a.branch_id,a.section,e.name AS dept,b.name AS course,c.name AS branch,f.session_yr,f.`session`
FROM reg_regular_form a
INNER JOIN cbcs_courses b ON a.course_id=b.id
INNER JOIN cbcs_branches c ON a.branch_id=c.id
INNER JOIN user_details d ON a.admn_no=d.id
INNER JOIN cbcs_departments e ON d.dept_id=e.id
INNER JOIN final_semwise_marks_foil_freezed f ON a.admn_no=f.admn_no
WHERE a.admn_no='$admn_no'
ORDER BY f.semester desc, f.actual_published_on DESC 
LIMIT 1";
       }else{
       $sql="SELECT a.form_id,a.admn_no, CONCAT_WS(' ',d.first_name,d.middle_name,d.last_name) AS stu_name,a.semester,a.course_id,a.branch_id,a.section,e.name AS dept,b.name AS course,c.name AS branch
FROM reg_regular_form a
INNER JOIN cbcs_courses b ON a.course_id=b.id
INNER JOIN cbcs_branches c ON a.branch_id=c.id
INNER JOIN user_details d ON a.admn_no=d.id
INNER JOIN cbcs_departments e ON d.dept_id=e.id
WHERE a.admn_no='$admn_no' AND a.session_year='$session_year' AND a.`session`='$session' AND a.semester='$semester' AND a.course_id='$course' AND a.branch_id='$branch' AND d.dept_id='$dept'";
}
      ...
      if($course_id=="jrf"){
        ...
        GROUP BY x.admn_no, IF( /*x.semester<= x.reg_sem and*/ x.session_yr>='2019-2020' ,x.session_yr,null) ,  IF( /*x.semester<= x.reg_sem and*/ x.session_yr>='2019-2020' ,x.session,null) /*having  x.semester<= x.reg_sem*/
        ...
        fd.admn_no=y.admn_no ORDER BY y.actual_published_on,fd.sub_code,fd.cr_pts DESC LIMIT 10000000000) f GROUP BY  f.sub_code) f
        GROUP BY f.semester
        ";
      }else{
        //echo 'entered_2';
    //   $sql="SELECT GROUP_CONCAT(f.sub_code) AS subject, GROUP_CONCAT(CONCAT_WS('-',f.sub_code,f.cr_hr,f.total,f.grade,f.cr_pts,f.stored_ctotcrpts,f.stored_ctotcrhr,f.stored_core_ctotcrpts,f.stored_core_ctotcrhr,f.gpa,f.core_gpa,f.cgpa,f.core_cgpa,f.core_status,f.defaultremarks)) AS sub_details, GROUP_CONCAT((CASE WHEN f.grade='F' THEN f.sub_code END)) AS fail_subs,f.*
    //   FROM (select * from(
    //   SELECT y.foil_id, y.exam_type, y.session_yr,y.session,y.dept,y.course,y.branch,y.semester,fd.sub_code, fd.grade, fd.cr_pts, fd.cr_hr, fd.mis_sub_id,(CASE WHEN (fd.remark2 IS NULL) THEN 'N' ELSE fd.remark2 END) AS defaultremarks,(CASE WHEN (fd.total IS NULL OR fd.total='0') THEN '0' ELSE fd.total END) AS total, y.admn_no, y.ctotcrpts AS stored_ctotcrpts,y.ctotcrhr AS stored_ctotcrhr,y.core_ctotcrpts AS stored_core_ctotcrpts,y.core_ctotcrhr AS stored_core_ctotcrhr, y.actual_published_on,y.gpa,y.core_gpa,y.cgpa,y.core_cgpa,y.core_status, y.type,y.priority_tailored
    //   FROM (
    //   SELECT x.*
    //   FROM (
    //   SELECT a.type,a.exam_type, (CASE WHEN (a.session='Monsoon' AND a.`type`='R') THEN '1' WHEN (a.session='Monsoon' AND a.`type`='O') THEN '2' WHEN (a.session='Monsoon' AND a.`type`='S') THEN '3' WHEN a.session='Winter' AND a.`type`='R' THEN '4' WHEN a.session='Winter' AND a.`type`='O' THEN '5' WHEN a.session='Winter' AND a.`type`='S' THEN '6' WHEN(a.session='Summer' AND a.`type`='R') THEN '7' END) AS priority_tailored,a.session_yr,a.session,a.admn_no,a.dept,a.course,a.branch,a.semester,a.id AS foil_id,a.`status`, a.ctotcrpts,a.ctotcrhr, a.core_ctotcrpts,a.core_ctotcrhr, a.tot_cr_hr,a.tot_cr_pts, a.core_tot_cr_hr,a.core_tot_cr_pts, if(rg.semester<>a.semester, a.semester, NULL) AS sem, rg.semester AS reg_sem, a.published_on, a.actual_published_on,a.gpa,a.core_gpa,a.cgpa,a.core_cgpa,a.core_status
    //   FROM final_semwise_marks_foil_freezed AS a
    //   JOIN reg_regular_form rg ON rg.admn_no=a.admn_no AND rg.hod_status='1' AND rg.acad_status='1' AND rg.session_year>='2019-2020' AND rg.`session`='Monsoon'
    //   JOIN mis_session ms ON a.`session`= ms.`session`
    //   JOIN mis_session_year msy ON a.session_yr = msy.session_year
    //   WHERE a.admn_no='$admn_no' AND UPPER(a.course)<>'MINOR' AND (a.semester!= '0' AND a.semester!='-1') AND a.course<>'jrf'
    //   ORDER BY a.admn_no,a.session_yr,priority_tailored, a.actual_published_on DESC
    //   LIMIT 100000000)x
    //   GROUP BY x.admn_no,x.session_yr,x.session,x.semester,x.type
    //   ORDER BY x.admn_no,x.session_yr,x.priority_tailored,x.semester
    //   LIMIT 100000000) y
    //   JOIN final_semwise_marks_foil_desc_freezed fd ON fd.foil_id=y.foil_id AND fd.admn_no=y.admn_no AND (CASE WHEN y.session = 'Summer' THEN fd.current_exam = 'Y' WHEN y.session = 'Winter' AND y.exam_type != 'R' THEN fd.current_exam = 'Y' WHEN y.session = 'Monsoon' AND y.exam_type != 'R' THEN fd.current_exam = 'Y'  ELSE 1=1 END) ORDER BY y.actual_published_on,fd.sub_code,fd.cr_pts DESC LIMIT 10000000000) f Group by f.sub_code)f
    //   GROUP BY f.admn_no,f.session_yr,f.session,(CASE WHEN f.session='Summer' THEN 1=1 WHEN f.session='Winter' THEN 1=1 WHEN f.session='Monsoon' THEN 1=1 ELSE f.semester END)
    //   ORDER BY f.admn_no,f.session_yr,f.priority_tailored,f.semester
//";

$sql = "select * from ((SELECT GROUP_CONCAT(f.sub_code) AS subject, GROUP_CONCAT(CONCAT_WS('-',f.sub_code,f.cr_hr,f.total,f.grade,f.cr_pts,f.stored_ctotcrpts,f.stored_ctotcrhr,f.gpa)) AS sub_details, GROUP_CONCAT((CASE WHEN f.grade='F' THEN f.sub_code END)) AS fail_subs,f.*
FROM(
SELECT y.foil_id, y.examtype, y.ysession AS session_yr,y.wsms AS session,y.deptmis AS dept,y.course,y.branch,y.sem AS semester,y.subje_code AS sub_code,y.grade,y.crpts AS cr_pts,y.crdhrs AS cr_hr, NULL AS mis_sub_id, NULL AS defaultremarks, (CASE WHEN (y.totalmarks IS NULL OR y.totalmarks ='0') THEN '0' ELSE y.totalmarks END) AS total, y.adm_no AS admn_no, y.ctotcrpts AS stored_ctotcrpts,y.ctotcrhr AS stored_ctotcrhr, NULL AS stored_core_ctotcrpts, NULL AS stored_core_ctotcrhr, NULL AS actual_published_on,y.gpa, NULL AS core_gpa, NULL AS cgpa, NULL AS core_cgpa, NULL AS core_status, NULL AS TYPE,y.priority_tailored
FROM (
SELECT x.*
FROM(
SELECT a.examtype,a.sem_code,a.subje_code,(CASE WHEN (a.wsms='MS' AND a.`examtype`='R') THEN '1' WHEN (a.wsms='MS' AND a.`examtype`='O') THEN '2' WHEN (a.wsms='MS' AND a.`examtype`='S') THEN '3' WHEN a.wsms='WS' AND a.`examtype`='R' THEN '4' WHEN a.wsms='WS' AND a.`examtype`='O' THEN '5' WHEN a.wsms='WS' AND a.`examtype`='S' THEN '6' WHEN(a.wsms='ZS' AND a.`examtype`='R') THEN '7' END) AS priority_tailored,a.ysession,a.wsms,a.adm_no,dsem.deptmis,dsem.course,dsem.branch,dsem.sem,a.id AS foil_id,a.ctotcrpts,a.ctotcrhr,a.totcrhr,a.totcrpts,ROUND(CAST(a.gpa AS DECIMAL(10,2)),2) AS gpa,a.grade,a.crpts,a.crdhrs,a.totalmarks
FROM tabulation1 AS a
INNER JOIN dip_m_semcode dsem ON a.sem_code = dsem.semcode
WHERE a.adm_no='$admn_no' AND UPPER(dsem.course)<>'MINOR' AND (dsem.sem!= '0' AND dsem.sem!='-1') AND dsem.course<>'jrf' AND (CASE WHEN a.wsms = 'ZS' THEN a.subje_pf = 'SP' WHEN a.wsms = 'WS' AND a.examtype != 'R' THEN a.subje_pf = 'SP' WHEN a.wsms = 'MS' AND a.examtype != 'R' THEN a.subje_pf = 'SP' ELSE 1=1 END)
ORDER BY a.adm_no,a.ysession DESC
LIMIT 100000000)x
GROUP BY x.ysession, x.sem_code,x.examtype, x.wsms, x.subje_code
ORDER BY x.ysession, x.sem_code, x.wsms, x.examtype
LIMIT 100000000)y)f
GROUP BY f.admn_no,f.session_yr,f.session,(CASE WHEN f.session='ZS' THEN 1=1 WHEN f.session='WS' THEN 1=1 WHEN f.session='MS' THEN 1=1 ELSE f.semester END)
ORDER BY f.admn_no,f.session_yr,f.session,f.priority_tailored,f.semester)
UNION ALL
(SELECT GROUP_CONCAT(f.sub_code) AS subject, GROUP_CONCAT(CONCAT_WS('-',f.sub_code,f.cr_hr,f.total,f.grade,f.cr_pts,f.stored_ctotcrpts,f.stored_ctotcrhr,f.stored_core_ctotcrpts,f.stored_core_ctotcrhr,f.gpa,f.core_gpa,f.cgpa,f.core_cgpa,f.core_status,f.defaultremarks)) AS sub_details, GROUP_CONCAT((CASE WHEN f.grade='F' THEN f.sub_code END)) AS fail_subs,f.*
FROM (
SELECT *
FROM(
SELECT y.foil_id, y.exam_type, y.session_yr,y.session,y.dept,y.course,y.branch,y.semester,fd.sub_code, fd.grade, fd.cr_pts, fd.cr_hr, fd.mis_sub_id,(CASE WHEN (fd.remark2 IS NULL) THEN 'N' ELSE fd.remark2 END) AS defaultremarks,(CASE WHEN (fd.total IS NULL OR fd.total='0') THEN '0' ELSE fd.total END) AS total, y.admn_no, y.ctotcrpts AS stored_ctotcrpts,y.ctotcrhr AS stored_ctotcrhr,y.core_ctotcrpts AS stored_core_ctotcrpts,y.core_ctotcrhr AS stored_core_ctotcrhr, y.actual_published_on,y.gpa,y.core_gpa,y.cgpa,y.core_cgpa,y.core_status, y.type,y.priority_tailored
...
SELECT x.*
FROM (
SELECT a.type,a.exam_type, (CASE WHEN (a.session='Monsoon' AND a.`type`='R') THEN '1' WHEN (a.session='Monsoon' AND a.`type`='O') THEN '2' WHEN (a.session='Monsoon' AND a.`type`='S') THEN '3' WHEN a.session='Winter' AND a.`type`='R' THEN '4' WHEN a.session='Winter' AND a.`type`='O' THEN '5' WHEN a.session='Winter' AND a.`type`='S' THEN '6' WHEN(a.session='Summer' AND a.`type`='R') THEN '7' END) AS priority_tailored,a.session_yr,a.session,a.admn_no,a.dept,a.course,a.branch,a.semester,a.id AS foil_id,a.`status`, a.ctotcrpts,a.ctotcrhr, a.core_ctotcrpts,a.core_ctotcrhr, a.tot_cr_hr,a.tot_cr_pts, a.core_tot_cr_hr,a.core_tot_cr_pts, if(rg.semester<>a.semester, a.semester, NULL) AS sem, rg.semester AS reg_sem, a.published_on, a.actual_published_on,ROUND(CAST(a.gpa AS DECIMAL(10,2)),2) AS gpa,a.core_gpa,ROUND(CAST(a.cgpa AS DECIMAL(10,2)),2) AS cgpa,a.core_cgpa,a.core_status
FROM final_semwise_marks_foil_freezed AS a
JOIN reg_regular_form rg ON rg.admn_no=a.admn_no AND rg.hod_status='1' AND rg.acad_status='1' AND rg.session_year>='2019-2020' AND rg.`session`='Monsoon'
JOIN mis_session ms ON a.`session`= ms.`session`
JOIN mis_session_year msy ON a.session_yr = msy.session_year
...
ORDER BY x.admn_no,x.session_yr,x.priority_tailored,x.semester
LIMIT 100000000) y
JOIN final_semwise_marks_foil_desc_freezed fd ON fd.foil_id=y.foil_id AND fd.admn_no=y.admn_no AND (CASE WHEN y.session = 'Summer' THEN fd.current_exam = 'Y' WHEN y.session = 'Winter' AND y.exam_type != 'R' THEN fd.current_exam = 'Y' WHEN y.session = 'Monsoon' AND y.exam_type != 'R' THEN fd.current_exam = 'Y' ELSE 1=1 END)
ORDER BY y.actual_published_on,fd.sub_code,fd.cr_pts DESC
LIMIT 10000000000) f
GROUP BY f.session_yr,f.session,f.sub_code)f
GROUP BY f.admn_no,f.session_yr,f.session,(CASE WHEN f.session='Summer' THEN 1=1 WHEN f.session='Winter' THEN 1=1 WHEN f.session='Monsoon' THEN 1=1 ELSE f.semester END)
ORDER BY f.admn_no,f.session_yr,f.priority_tailored,f.semester)) ttt order by ttt.session_yr,ttt.foil_id";
    ...
    public function get_total_credit_point_policy($course){
        ...
        $sql = "SELECT a.course_comp,a.course_credit_min,a.course_credit_max FROM cbcs_curriculam_policy a
        WHERE a.cbcs_credit_points_policy_id=(SELECT a.id FROM cbcs_credit_points_policy a WHERE a.course_id='$course');";
    ...
    public function get_student_credit_point_for_mtech_mba($admn_no,$course){

        $sql = "SELECT ttt.* from (SELECT bb.*,aa.* FROM(
            SELECT zz3.stu_template4,SUM(zz3.cr_hr)AS credit_point FROM(
            SELECT zz2.*,
            SUBSTRING(zz2.stu_template3,1,CHAR_LENGTH(zz2.stu_template3) - 1) AS stu_template4
            FROM(
            SELECT zz1.* ,
            CASE WHEN (zz1.sub_category_final='OE')THEN 'DE/OE1' ELSE zz1.stu_template2 END AS stu_template3

            FROM (
            SELECT zz.* ,
            SUBSTRING(zz.stu_template1,1,CHAR_LENGTH(zz.stu_template1) - 1) AS sub_category_final
            FROM
            (SELECT
            a.sub_code,a.cr_hr,
            b.sub_category,b.sub_category_cbcs_offered ,
            CASE WHEN (b.sub_category_cbcs_offered ='NA' || b.sub_category_cbcs_offered IS null || b.sub_category_cbcs_offered ='')
            THEN b.sub_category ELSE b.sub_category_cbcs_offered END AS stu_template1,
            c.credit_hours,c.id,c.semester,c.sub_category AS cso_sub_category,c.unique_sub_pool_id,
            CASE WHEN (c.unique_sub_pool_id='NA')THEN c.sub_category ELSE c.unique_sub_pool_id END AS stu_template2
            FROM final_semwise_marks_foil_desc_freezed a
            INNER JOIN cbcs_stu_course b ON b.subject_code=a.sub_code AND a.admn_no=b.admn_no
            INNER JOIN cbcs_subject_offered c ON c.id=b.sub_offered_id
            WHERE a.admn_no='$admn_no'
            )zz
            GROUP BY zz.sub_code
            ORDER BY zz.semester,zz.stu_template1
            )zz1
            )zz2
            )zz3
            GROUP BY zz3.stu_template4)bb
            right join
            (SELECT a.course_comp,a.course_credit_min FROM cbcs_curriculam_policy a
            WHERE a.cbcs_credit_points_policy_id=(SELECT a.id FROM cbcs_credit_points_policy a WHERE a.course_id='$course'))aa
            ON bb.stu_template4=aa.course_comp) ttt WHERE ttt.course_credit_min!=0";
    ...
    public function get_student_credit_point_for_msc($admn_no,$course){
        ...
        $sql = "SELECT ttt.* from (SELECT bb.*,aa.* FROM(
            ...
            zz1.stu_template2 AS stu_template3
            FROM (
            SELECT zz.* ,
            SUBSTRING(zz.stu_template1,1,CHAR_LENGTH(zz.stu_template1) - 1) AS sub_category_final
            FROM
...
function count_pass_subject($admn_no){
  $sql="SELECT  COUNT(p.sub_code)AS cnt FROM(
SELECT a.*
FROM final_semwise_marks_foil_desc_freezed a
WHERE a.admn_no='$admn_no'
#AND a.foil_id=?
AND a.grade NOT IN ('F','I')
