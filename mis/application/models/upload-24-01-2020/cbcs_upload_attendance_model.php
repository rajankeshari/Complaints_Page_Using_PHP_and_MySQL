    ...
    function get_student($data){
      ...
      if($data['course_id']=='comm'){
        ...
        $p=array($data['branch_id'],$data['sub_code'],$data['session_year'],$data['session']);
			...
			$con=" a.course=?";
        $p=array($data['course_id'],$data['sub_code'],$data['session_year'],$data['session']);
...
/*  function getstartgroup($subid, $groupno, $sessionid = null, $map = null) {
  ...
  //====================================06-01-2020 END=========================
	  ...
	  function get_no_of_student($syear,$sess,$cid,$bid,$scode,$p1,$p2,$sec="",$rstatus,$sub_category){
		  if($p1=='2' && $p2=='2'){
...
FROM reg_regular_form a
INNER JOIN ".$tbl." b ON b.form_id=a.form_id AND b.admn_no=a.admn_no
WHERE a.session_year=? AND a.`session`=? AND b.course=? AND b.subject_code=? ".$con;
       $query = $this->db->query($sql, array($syear,$sess,$cid,$scode,$p1,$p2));
