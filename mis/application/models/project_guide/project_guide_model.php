...
Class Project_guide_model extends CI_Model {
    ...
    public function get_many_records($table_name = '', $filters_id_values = "", $request_fields = "", $order_by = '') {
		...
		if($table_name!='dept_auth'){
        if ($order_by) {
			$order_by='academic_year,admn_no';
    ...
    public function get_records_from_two_join($tableOne, $tableOneColumn, $tableTwo, $tableTwoColumn, $filters_id_values, $orderBy = "", $req_data = "") {
		...
		$this->db->join("users c", "c.id=a.$tableOneColumn", "inner");
	...
	function get_user_status($admn_no){
	...
	function get_stu_details($admn_no){
		$q = "SELECT * FROM stu_details WHERE admn_no=?";
	...
	function get_phd_awarded($admn_no){
		$q = "SELECT * FROM phd_awarded WHERE admn_no=?";
	...
	function get_drop_student_status($admn_no){
		$q = "SELECT * from drop_student_status WHERE admn_no=?";
	...
	function get_part_full_time($admn_no){
		$q = " SELECT * FROM stu_academic WHERE admn_no=?";

        $qu = $this->db->query($q,array($admn_no));
        
           
        if ($qu->num_rows() > 0){
            return $qu->row()->other_rank;
        }else{
            return false;    
        }
	}
