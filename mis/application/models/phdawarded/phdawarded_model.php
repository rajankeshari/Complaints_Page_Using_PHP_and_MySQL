...
where e.status='P'";
 ...
 function get_address($id,$type){
  ...
  //=====================================================Student Admission Date Change =======================================
 function add_student_admission_change($data){
        if($this->db->insert('stu_edit_admission_no',$data))
			return $this->db->insert_id();
		else
			return FALSE;
	...
	function check_student_admissionYear($id){
        $sql="select * from stu_details a where a.admn_no=?";
        $query = $this->db->query($sql,array($id));
        if ($this->db->affected_rows() > 0) {
            return $query->row();
        } else {
	...
	function update_student_admission_number($admn_date,$admn_no){
	 //$sql = "UPDATE stu_details SET admn_date=? WHERE admn_no=?";
		$sql="UPDATE stu_details SET admn_date=STR_TO_DATE('$admn_date', '%Y-%m-%d') WHERE admn_no=?";
        $query = $this->db->query($sql,array($admn_no));
 ...
 function get_student_emailid($id){
        $sql="SELECT a.domain_name FROM emaildata a WHERE a.admission_no=?";
        $query = $this->db->query($sql,array($id));
        if ($this->db->affected_rows() > 0) {
            return $query->row();
        } else {
