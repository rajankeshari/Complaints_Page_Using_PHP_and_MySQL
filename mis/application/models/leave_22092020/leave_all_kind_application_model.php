...
class Leave_all_kind_application_model extends CI_Model {
	...
	function get_flow_of_emp($ref_id){//written on 23.01.2020
		$myquery = "select id,salutation,first_name,middle_name,last_name  from user_details where id in (select submitted_by from leave_auth_table where ref_id='$ref_id')";
        $query = $this->db->query($myquery);
