<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	Class Employee_pass_auth_model extends CI_Model
	{
		var $table1 = 'user_details';
		var $table2 = 'password_change_count';
		var $table3 = 'users';
		var $table4 = 'user_auth_types';
		...
		{
			$query = $this->db->get_where($this->table4,array('id' => $id ));

			foreach ($query->result() as $row) {

				if($row->auth_id==='admin')return true;
