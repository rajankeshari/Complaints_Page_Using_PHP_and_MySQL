...
INNER JOIN cbcs_optional_mapping_desc b ON b.map_id=a.id
WHERE a.session_year=? AND a.`session`=? AND b.emp_no=?
GROUP BY a.sub_code,a.group_no,b.section)t
INNER JOIN cbcs_subject_offered t1 ON t1.sub_code=t.sub_code AND t1.session_year=t.session_year AND t1.session=t.session
GROUP BY t.sub_code,t.group_no) UNION (
...
GROUP BY sub_code,sub_group,section)p,(
...
WHERE b.session_year=? AND b.`session`=? AND a.subject_code=? and sub_category_cbcs_offered=? AND b.hod_status='1' AND b.acad_status='1'
ORDER BY c.id) union (SELECT c.id, c.first_name,c.middle_name,c.last_name
...
ORDER BY c.id)" ;
        ...
        $query = $this->db->query($sql, array($data['session_year'],$data['session'],$data['sub_code'],$data['section'],$data['session_year'],$data['session'],$data['sub_code'],$data['section']));
    }
    else{
...
ORDER BY c.id)" ;
        ...
        $query = $this->db->query($sql, array($data['session_year'],$data['session'],$data['sub_code'],$data['session_year'],$data['session'],$data['sub_code']));
...
//echo $this->db->last_query();die();
    ...
    //attenance generate starts non common
    function get_student_generate($data){
          $sql = "
(SELECT c.id, c.first_name,c.middle_name,c.last_name
...
INNER JOIN user_details c ON c.id=b.admn_no
INNER JOIN users d ON d.id=c.id AND d.`status`='A'
WHERE b.session_year=? AND b.`session`=? AND a.subject_code=? AND b.hod_status='1' AND b.acad_status='1' and (a.remark2='1' || a.remark2='3')
ORDER BY c.id)" ;
...
//echo $this->db->last_query();die();
    ...
    function get_student_generate_common($data){
          $sql = "
(SELECT c.id, c.first_name,c.middle_name,c.last_name
FROM pre_stu_course a
INNER JOIN reg_regular_form b ON b.form_id=a.form_id AND b.admn_no=a.admn_no
INNER JOIN user_details c ON c.id=b.admn_no
INNER JOIN users d ON d.id=c.id AND d.`status`='A'
WHERE b.session_year=? AND b.`session`=? AND a.subject_code=? and a.sub_category_cbcs_offered=? AND b.hod_status='1' AND b.acad_status='1' and (a.remark2='1' || a.remark2='3')
ORDER BY c.id)" ;

        $query = $this->db->query($sql, array($data['session_year'],$data['session'],$data['sub_code'],$data['section']));
//echo $this->db->last_query();die();
        if ($this->db->affected_rows() > 0) {
            return $query->result();
        } else {
    ...
    //attendance generate ends non common
...
if($section=='A' || $section=='B' || $section=='C' || $section=='D' || $section=='E' || $section=='F' || $section=='G' || $section=='H'){
  ...
  function get_absent_details($syear,$sess,$sub_code,$group,$section,$cid){
    ...
    if($cid=='comm'){
        $sql = "SELECT  *  FROM cbcs_class_engaged_course_wise WHERE session_year=? and session=? and course_code=? and  group_no=?    and section =?     ORDER BY date /*DESC LIMIT 1*/";
         $query = $this->db->query($sql, array($syear,$sess,$sub_code,$group,$section));
        
    }else{
        
        $sql = "SELECT  *  FROM cbcs_class_engaged_course_wise WHERE session_year=? and session=? and course_code=? and  group_no=?   ORDER BY date ";
...
function get_student_groupwise($session_year,$session,$sub_code,$course_id,$branch_id,$start1,$end,$sub_offered_id){
...
AND b.course_id=? AND b.branch_id=? AND a.sub_offered_id=?
...
ORDER BY c.id  LIMIT ?,?)  " ;
        ...
        $query = $this->db->query($sql, array($session_year,$session,$sub_code,$course_id,$branch_id,$sub_offered_id,(int)$start,(int)$end,$session_year,$session,$sub_code,$course_id,$branch_id,$sub_offered_id,(int)$start,(int)$end));
...
//for attendance generate starts

function get_student_groupwise_generate($session_year,$session,$sub_code,$course_id,$branch_id,$start1,$end,$sub_offered_id){
...
ORDER BY c.id  LIMIT ?,?) 
  " ;
        ...
        $query = $this->db->query($sql, array($session_year,$session,$sub_code,$course_id,$branch_id,$sub_offered_id,(int)$start,(int)$end));
