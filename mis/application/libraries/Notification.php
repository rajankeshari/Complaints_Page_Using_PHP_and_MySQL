<?php if(!defined("BASEPATH")){ exit("No direct script access allowed"); }
	...
	class Notification {
		...
		function notify($user_id_to, $auth, $title, $description, $path, $type = "")
		{
			...
			$data['notice_title'] = $this->CI->authorization->strclean($title);
			$data['description'] = $this->CI->authorization->strclean($description);
			$data['notice_path'] = $this->CI->authorization->strclean($path);
			$data['notice_type'] = $this->CI->authorization->strclean($type);
